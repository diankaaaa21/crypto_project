    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            self.write(
                dict(message=f"Restarting '{workername}' worker's pool"))
        else:
...
class WorkerPoolGrow(ControlHandler):
    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            self.write(
                dict(message=f"Growing '{workername}' worker's pool by {n}"))
        else:
            logger.error(response)
            self.set_status(403)
            reason = self.error_reason(workername, response)
            self.write(
                f"Failed to grow '{workername}' worker's pool: {reason}")
...
class WorkerPoolShrink(ControlHandler):
    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            self.write(
                dict(message=f"Shrinking '{workername}' worker's pool by {n}"))
        else:
            logger.error(response)
            self.set_status(403)
            reason = self.error_reason(workername, response)
            self.write(
                f"Failed to shrink '{workername}' worker's pool: {reason}")
    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            self.write(
                dict(
                    message=f"Autoscaling '{workername}' worker "
                    "(min={min}, max={max})"
                )
            )
        else:
    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            ...
            reason = self.error_reason(workername, response)
            self.write(
                f"Failed to add '{queue}' consumer to '{workername}' worker: {reason}"
            )
    ...
    def post(self, workername):
        ...
        **Example response**:
        ...
        if response and "ok" in response[0][workername]:
            ...
            reason = self.error_reason(workername, response)
            self.write(
                f"Failed to cancel '{queue}' consumer from '{workername}' worker: {reason}"
            )
    ...
    def post(self, taskname):
        ...
        **Example response**:
        ...
        response = self.capp.control.time_limit(
            taskname, reply=True, hard=hard, soft=soft, destination=destination
        )
