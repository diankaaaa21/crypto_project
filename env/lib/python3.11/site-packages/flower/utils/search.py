...
def parse_search_terms(raw_search_value):
    ...
    for query_part in re.findall(search_regexp, raw_search_value):
        ...
        if query_part.startswith("result:"):
            parsed_search["result"] = preprocess_search_value(
                query_part[len("result:"):]
            )
        elif query_part.startswith("args:"):
            if "args" not in parsed_search:
                parsed_search["args"] = []
            parsed_search["args"].append(
                preprocess_search_value(query_part[len("args:"):])
            )
            ...
            try:
                key, value = [
                    p.strip() for p in query_part[len("kwargs:"):].split("=")
                ]
            except ValueError:
                continue
            ...
            if "state" not in parsed_search:
                parsed_search["state"] = []
            parsed_search["state"].append(
                preprocess_search_value(query_part[len("state:"):])
            )
        else:
...
def satisfies_search_terms(task, search_terms):
    ...
    terms = [
        ...
        kwargs_search_terms
        and all(
            stringified_dict_contains_value(k, v, task.kwargs)
            for k, v in kwargs_search_terms.items()
        ),
        args_search_terms
        and task_args_contains_search_args(task.args, args_search_terms),
    ]
    return any(terms)
