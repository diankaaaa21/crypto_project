...
class BrokerView(BaseHandler):
    ...
    async def get(self):
        ...
        try:
            broker = Broker(
                app.capp.connection(connect_timeout=1.0).as_uri(
            ...
            raise web.HTTPError(
                404, f"'{app.transport}' broker is not supported"
            ) from exc
        ...
        self.render(
            "broker.html", broker_url=app.capp.connection().as_uri(), queues=queues
        )
