...
def _transform_attrs(
    ...
    own_attrs = [fca(attr_name, ca, anns.get(attr_name))
...
class _ClassBuilder:
    ...
    def __init__(
        ...
        self._script_snippets: list[tuple[str,
                                          dict, Callable[[dict, dict], Any]]] = []
...
def _make_repr_script(attrs, ns) -> tuple[str, dict]:
    ...
    globs = {name + "_repr": r for name, r,
             _ in attr_names_with_reprs if r != repr}
    ...
    for name, r, i in attr_names_with_reprs:
        accessor = "self." + \
            name if i else 'getattr(self, "' + name + '", NOTHING)'
        fragment = (
            "%s={%s!r}" % (name, accessor)
            if r == repr
...
def make_class(name, attrs, bases=(object,), class_body=None, **attributes_arguments):
    ...
    with contextlib.suppress(AttributeError, ValueError):
        type_.__module__ = sys._getframe(
    ...
    cls.__annotations__ = {k: v.type for k,
                           v in cls_dict.items() if v.type is not None}
    return cls
    ...
    *last* value.
    ...
    if return_instance:
        ...
        def pipe_converter(val, inst, field):
            for c in converters:
                val = c(val, inst, field) if isinstance(
                    c, Converter) else c(val)
