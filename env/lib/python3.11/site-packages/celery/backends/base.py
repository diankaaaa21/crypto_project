...
class Backend:
        ...
        if exc_module is None:
            ...
            try:
                ...
                cls = create_exception_cls(
                    exc_type, celery.exceptions.__name__)
            ...
            try:
                ...
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        raise_with_context(
                            BackendGetMetaError(
                                "failed to get meta", task_id=task_id),
                        )
                else:
                    raise
    ...
    def fallback_chord_unlock(self, header_result, body, countdown=1, **kwargs):
        ...
        priority = body.options.get(
            "priority", getattr(body_type, "priority", 0))
...
class BaseKeyValueStoreBackend(Backend):
    ...
    def get_key_for_task(self, task_id, key=""):
        ...
        if not task_id:
            raise ValueError(
                f"task_id must not be empty. Got {task_id} instead.")
    ...
    def get_key_for_group(self, group_id, key=""):
        ...
        if not group_id:
            raise ValueError(
                f"group_id must not be empty. Got {group_id} instead.")
    ...
    def _store_result(
        ...
        try:
            ...
            raise BackendStoreError(
                str(ex), state=state, task_id=task_id) from ex
