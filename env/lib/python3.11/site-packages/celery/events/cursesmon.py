...
class CursesMonitor:  # pragma: no cover
    ...
    keyalias = {curses.KEY_DOWN: "J",
                curses.KEY_UP: "K", curses.KEY_ENTER: "I"}
    ...
    def alert(self, callback, title=None):
        ...
        if title:
            self.win.addstr(next(y), 3, title, curses.A_BOLD |
                            curses.A_UNDERLINE)
            next(blank_line)
        callback(my, mx, next(y))
        self.win.addstr(
            my - 1, 0, "Press any key to continue...", curses.A_BOLD)
        self.win.refresh()
        while 1:
            try:
    ...
    def selection_rate_limit(self):
        ...
        if rlimit:
            reply = self.app.control.rate_limit(
                task.name, rlimit.strip(), reply=True)
    ...
    def selection_result(self):
        ...
        def alert_callback(my, mx, xs):
            y = count(xs)
            task = self.state.tasks[self.selected_task]
            result = getattr(task, "result", None) or getattr(
                task, "exception", None)
        ...
        with self.lock:
            ...
            if self.selected_task:
                ...
                try:
                    ...
                    info = " ".join(f"{key}={value}" for key,
                                    value in info.items())
                    detail = "... -> key i"
                infowin = abbr(
                    info, self.screen_width -
                    len(self.selected_str) - 2, detail
                )
            ...
            self.safe_add_str(my - 2, x + len(self.help_title),
