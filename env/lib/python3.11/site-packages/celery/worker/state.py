...
SUCCESSFUL_EXPIRES = float(os.environ.get(
    "CELERY_WORKER_SUCCESSFUL_EXPIRES", 10800))
...
successful_requests = LimitedSet(
    maxlen=SUCCESSFUL_MAX, expires=SUCCESSFUL_EXPIRES)
...
C_BENCH_EVERY = int(
    os.environ.get("C_BENCH_EVERY") or os.environ.get(
        "CELERY_BENCH_EVERY") or 1000
)
    ...
    if current_process()._name == "MainProcess":
        ...
        def on_shutdown():
            if bench_first is not None and bench_last is not None:
                print(
                    "- Time spent in benchmark: {!r}".format(
