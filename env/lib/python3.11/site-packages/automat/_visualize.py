...
from ._typed import Core, InputProtocol, TypeMachine
...
def makeDigraph(
    ...
    for n, eachTransition in enumerate(automaton.allTransitions()):
        ...
        digraph.node(thisTransition, label=_gvhtml(
            table), margin="0.2", shape="none")
...
def tool(
    ...
    for fqpn, machine in _findMachines(args.fqpn):
        ...
        if explicitlySaveDot:
            digraph.save(filename="{}.dot".format(
                fqpn), directory=args.dot_directory)
