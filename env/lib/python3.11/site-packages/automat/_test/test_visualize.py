...
class ElementMakerTests(TestCase):
    ...
    def test_sortsAttrs(self):
        ...
        self.assertEqual(expected, self.elementMaker(
            "div", b="2", a="1", c="3"))
...
class TableMakerTests(TestCase):
    ...
    def setUp(self):
        ...
        self.port = "the port"
        self.tableMaker = functools.partial(
            tableMaker, _E=self.fakeElementMaker)
    ...
    def test_inputLabelRow(self):
        ...
        for outputLabels in ([], ["an output label"]):
            table = self.tableMaker(
                self.inputLabel, outputLabels, port=self.port)
            ...
            self.assertEqual(findElements(
                inputLabelRow, isLeaf), [self.inputLabel])
    ...
    def test_withOutputLabels(self):
        ...
        self.assertEqual(
            findElements(outputRow, isLeaf), [
                "output label 1", "output label 2"]
        )
...
class VisualizeToolTests(TestCase):
    ...
    def test_onlySaveDot(self):
        ...
        for arg in ("--image-directory", "-i"):
            ...
            self.assertFalse(
                any("image" in line for line in self.collectedOutput))
    ...
    def test_saveOnlyImage(self):
        ...
        for arg in ("--dot-directory", "-d"):
            ...
            self.assertFalse(
                any("dot" in line for line in self.collectedOutput))
    ...
    def test_saveDotAndImagesInSameDirectory(self):
        ...
        self.assertTrue(
            any("image and dot" in line for line in self.collectedOutput))
