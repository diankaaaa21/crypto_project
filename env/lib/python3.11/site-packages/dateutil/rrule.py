...
MDAY366MASK = tuple(
    M31 + M29 + M31 + M30 + M31 + M30 + M31 +
    M31 + M30 + M31 + M30 + M31 + M31[:7]
)
                ...
                if no1wkst:
                    ...
                    if -1 not in rr._byweekno:
                        ...
                        if lno1wkst >= 4:
                            lno1wkst = 0
                            lnumweeks = (
                                52 + (lyearlen + (lyearweekday - rr._wkst) %
                                      7) % 7 // 4
                            )
                        else:
    ...
    def htimeset(self, hour, minute, second):
        ...
        for minute in rr._byminute:
            for second in rr._bysecond:
                tset.append(datetime.time(
                    hour, minute, second, tzinfo=rr._tzinfo))
        tset.sort()
        return tset
...
class _rrulestr(object):
    ...
    def _parse_rfc(
        ...
        TZID_NAMES = dict(
            map(lambda x: (x.upper(), x), re.findall(
                "TZID=(?P<name>[^:]+):", s))
        )
        s = s.upper()
        if not s.strip():
