...
import typing
from typing import Any, Awaitable, Callable, Dict, List, Tuple, Type, Union, overload

if typing.TYPE_CHECKING:
    from typing import Deque, Iterable, Optional, Sequence, Set  # noqa: F401

_T = typing.TypeVar("_T")

_Yieldable = Union[
    None, Awaitable, List[Awaitable], Dict[Any,
                                           Awaitable], concurrent.futures.Future
]
...
def multi_future(
    ...
    assert all(is_future(i) or isinstance(i, _NullFuture)
               for i in children_futs)
    unfinished_children = set(children_futs)

    future = _create_future()
    if not children_futs:
        future_set_result_unless_cancelled(
            future, {} if keys is not None else [])
...
def with_timeout(
    ...
    if isinstance(future_converted, Future):
        ...
        future_add_done_callback(
            future_converted, lambda future: io_loop.remove_timeout(
                timeout_handle)
        )
    else:
