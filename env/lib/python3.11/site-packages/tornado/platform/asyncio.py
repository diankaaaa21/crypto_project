...
class BaseAsyncIOLoop(IOLoop):
    def initialize(  # type: ignore
        ...
        existing_loop = IOLoop._ioloop_for_asyncio.setdefault(
            asyncio_loop, self)
    ...
    def add_handler(
        ...
        if events & IOLoop.READ:
            self.selector_loop.add_reader(
                fd, self._handle_events, fd, IOLoop.READ)
            self.readers.add(fd)
        if events & IOLoop.WRITE:
            self.selector_loop.add_writer(
    ...
    def add_callback(self, callback: Callable, *args: Any, **kwargs: Any) -> None:
        ...
        try:
            call_soon(self._run_callback, functools.partial(
    ...
    def add_callback_from_signal(
        ...
        warnings.warn("add_callback_from_signal is deprecated",
                      DeprecationWarning)
        try:
            self.asyncio_loop.call_soon_threadsafe(
                self._run_callback, functools.partial(
                    callback, *args, **kwargs)
            )
        except RuntimeError:
            pass
...
class SelectorThread:
    ...
    def _start_select(self) -> None:
        ...
        with self._select_cond:
            ...
            self._select_args = (list(self._readers.keys()),
