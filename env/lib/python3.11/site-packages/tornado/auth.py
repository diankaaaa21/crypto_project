    ...
    * ``_OAUTH_ACCESS_TOKEN_URL``:  The service's access token url.
    ...
    def authorize_redirect(
        ...
        if client_secret is not None:
            warnings.warn("client_secret argument is deprecated",
                          DeprecationWarning)
        handler = cast(RequestHandler, self)
...
def _oauth10a_signature(
    ...
    base_string = "&".join(_oauth_escape(e) for e in base_elems)
    key_elems = [escape.utf8(urllib.parse.quote(
        consumer_token["secret"], safe="~"))]
    key_elems.append(
        escape.utf8(urllib.parse.quote(
            token["secret"], safe="~") if token else "")
    )
    key = b"&".join(key_elems)
