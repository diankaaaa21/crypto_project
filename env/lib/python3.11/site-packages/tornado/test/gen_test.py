...
class GenBasicTest(AsyncTestCase):
    ...
    def test_multi_exceptions(self):
        ...
        with self.assertRaises(RuntimeError):
            yield gen.Multi(
                [self.async_exception(RuntimeError(
                    "error 1")), self.async_future(2)]
            )
...
class WaitIteratorTest(AsyncTestCase):
    ...
    def finish_coroutines(self, iteration, futures):
        ...
        if iteration < 8:
            self.io_loop.add_callback(
                self.finish_coroutines, iteration + 1, futures)

    @gen_test
    def test_iterator(self):
        futures = [Future(), Future(), Future(), Future()
                   ]  # type: List[Future[int]]
        ...
        while not g.done():
            try:
                ...
                self.assertIs(g.current_future,
                              futures[0], "exception future invalid")
            else:
                if i == 0:
    ...
    def test_no_ref(self):
        ...
        yield gen.with_timeout(
            datetime.timedelta(seconds=0.1), gen.WaitIterator(
                gen.sleep(0)).next()
        )
