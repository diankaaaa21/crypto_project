...
class SecureCookieV1Test(unittest.TestCase):
    def test_round_trip(self):
        ...
        self.assertEqual(handler.get_signed_cookie(
            "foo", min_version=1), b"bar")

    def test_cookie_tampering_future_timestamp(self):
        handler = CookieTestRequestHandler()
        # this string base64-encodes to '12345678'
        handler.set_signed_cookie(
            "foo", binascii.a2b_hex(b"d76df8e7aefc"), version=1)
        ...
        with ExpectLog(gen_log, "Cookie timestamp in future"):
            self.assertTrue(handler.get_signed_cookie(
                "foo", min_version=1) is None)
...
class CookieTest(WebTestCase):
    def get_handlers(self):
        ...
        class SetCookieDomainHandler(RequestHandler):
            def get(self):
                ...
                # either (see bug #285)
                self.set_cookie("unicode_args", "blah",
                                domain="foo.com", path="/foo")
    ...
    def test_cookie_special_char(self):
        ...
        self.assertTrue(
            headers[2] in ('semicolon="a;b"; Path=/',
    ...
    def test_set_cookie_expires_days(self):
        ...
        header_expires = email.utils.parsedate_to_datetime(
            match.groupdict()["expires"])
        ...
        for key in self.request.arguments:
            ...
            for bvalue in self.request.arguments[key]:
                if type(bvalue) != bytes:
                    raise Exception(
                        "incorrect type for value: %r" % type(bvalue))
            for svalue in self.get_arguments(key):
                if type(svalue) != unicode_type:
                    raise Exception(
                        "incorrect type for value: %r" % type(svalue))
...
class TypeCheckHandler(RequestHandler):
    def prepare(self):
        ...
        self.check_type("get_signed_cookie",
                        self.get_signed_cookie("asdf"), bytes)
        self.check_type("get_cookie", self.get_cookie("asdf"), str)

        self.check_type("xsrf_token", self.xsrf_token, bytes)
        self.check_type("xsrf_form_html", self.xsrf_form_html(), str)

        self.check_type("reverse_url", self.reverse_url(
            "typecheck", "foo"), str)
    ...
    def check_type(self, name, obj, expected_type):
        ...
        if expected_type != actual_type:
            self.errors[name] = "expected %s, got %s" % (
...
class DecodeArgHandler(RequestHandler):
    ...
    def get(self, arg):
        ...
        self.write(
            {"path": describe(arg), "query": describe(self.get_argument("foo"))})
...
class RedirectHandler(RequestHandler):
    def get(self):
        if self.get_argument("permanent", None) is not None:
            self.redirect(
                "/", permanent=bool(int(self.get_argument("permanent"))))
...
<div class="entry">...</div>""",
    ...
    def test_decode_argument(self):
        ...
        data = json_decode(response.body)
        self.assertEqual(
            data, {"path": ["bytes", "c3a9"], "query": ["bytes", "c3a9"]})
    ...
    def test_reverse_url(self):
        self.assertEqual(self.app.reverse_url(
            "decode_arg", "foo"), "/decode_arg/foo")
        self.assertEqual(self.app.reverse_url(
            "decode_arg", 42), "/decode_arg/42")
        self.assertEqual(self.app.reverse_url(
            "decode_arg", b"\xe9"), "/decode_arg/%E9")
        self.assertEqual(
            self.app.reverse_url("decode_arg", "\u00e9"), "/decode_arg/%C3%A9"
        )
        self.assertEqual(
            self.app.reverse_url(
                "decode_arg", "1 + 1"), "/decode_arg/1%20%2B%201"
        )
...
<script src="/analytics.js"/>
    ...
    def test_optional_path(self):
        self.assertEqual(self.fetch_json(
            "/optional_path/foo"), {"path": "foo"})
    ...
    def test_web_redirect(self):
        response = self.fetch("/web_redirect_permanent",
                              follow_redirects=False)
        self.assertEqual(response.code, 301)
    ...
    def test_web_redirect_double_slash(self):
        response = self.fetch(
            "//web_redirect_double_slash", follow_redirects=False)
        self.assertEqual(response.code, 301)
        self.assertEqual(response.headers["Location"], "/web_redirect_newpath")
...
class StaticFileTest(WebTestCase):
    ...
    def test_static_compressed_files(self):
        ...
        self.assertTrue(
            response.headers.get(
                "Content-Type") in set(("text/xml", "application/xml"))
        )

    def test_static_url(self):
        response = self.fetch("/static_url/robots.txt")
        self.assertEqual(
            response.body, b"/static/robots.txt?v=" + self.robots_txt_hash)
    ...
    def test_absolute_version_exclusion(self):
        ...
        self.assertEqual(response.body, utf8(
            self.get_url("/") + "static/robots.txt"))
    ...
    def test_static_etag(self):
        ...
        self.assertEqual(
            utf8(response.headers.get("Etag")), b'"' +
            self.robots_txt_hash + b'"'
        )
    ...
    def test_static_invalid_range(self):
        response = self.get_and_head(
            "/static/robots.txt", headers={"Range": "asdf"})
        self.assertEqual(response.code, 200)
...
class HostMatchingTest(WebTestCase):
    ...
    def test_host_matching(self):
        ...
        self.app.add_handlers(
            "www.e.*e.com", [("/baz", HostMatchingTest.Handler,
                              {"reply": "[3]"})]
        )
...
class DateHeaderTest(SimpleHandlerTestCase):
    ...
    def test_date_header(self):
        response = self.fetch("/")
        header_date = email.utils.parsedate_to_datetime(
...
class ClearAllCookiesTest(SimpleHandlerTestCase):
    ...
    def test_clear_all_cookies(self):
        ...
        self.assertTrue(
            set_cookies[0].startswith(
                "baz=;") or set_cookies[0].startswith('baz="";')
        )
        self.assertTrue(
            set_cookies[1].startswith(
                "foo=;") or set_cookies[1].startswith('foo="";')
        )
...
class ExceptionHandlerTest(SimpleHandlerTestCase):
    class Handler(RequestHandler):
        ...
        def log_exception(self, typ, value, tb):
            if isinstance(value, PermissionError):
                app_log.warning(
                    "custom logging for PermissionError: %s", value.args[0])
            else:
        ...
        class MyModule(UIModule):
            def render(self, x):
                return "In MyModule(%s) with handler value %s." % (
                    x,
                    typing.cast(UIMethodUIModuleTest.Handler,
                                self.handler).value(),
                )
...
class BaseStreamingRequestFlowControlTest(object):
    ...
    def test_flow_control_fixed_body(self: typing.Any):
        response = self.fetch(
            "/", body="abcdefghijklmnopqrstuvwxyz", method="POST")
        response.rethrow()
        self.assertEqual(
            json_decode(response.body),
            dict(
...
class SignedValueTest(unittest.TestCase):
    ...
    def test_key_versioning_invalid_key(self):
        ...
        newkeys.pop(0)
        decoded = decode_signed_value(
            newkeys, "key", signed, clock=self.present)
...
class XSRFCookieKwargsTest(SimpleHandlerTestCase):
    ...
    def test_xsrf_httponly(self):
        ...
        if header_expires.tzinfo is None:
            header_expires = header_expires.replace(
...
class FinishExceptionTest(SimpleHandlerTestCase):
    ...
    def test_finish_exception(self):
        for u in ["/", "/?finish_value=1"]:
            ...
            self.assertEqual(
                'Basic realm="something"', response.headers.get(
                    "WWW-Authenticate")
            )
...
class URLSpecReverseTest(unittest.TestCase):
    def test_reverse(self):
        ...
        self.assertEqual(
            "/favicon.ico", url(r"^/favicon\.ico$", None).reverse())
...
class RedirectHandlerTest(WebTestCase):
    ...
    def test_redirect_with_appending_argument(self):
        ...
        self.assertEqual(response.code, 301)
        self.assertEqual(
            response.headers["Location"], "/dst2?foo=bar&foo2=bar2")
