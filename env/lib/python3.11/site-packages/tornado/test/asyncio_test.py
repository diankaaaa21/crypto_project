...
class AsyncIOLoopTest(AsyncTestCase):
    ...
    def test_asyncio_adapter(self):
        ...
        self.assertEqual(self.io_loop.run_sync(
            native_coroutine_without_adapter), 42)
        self.assertEqual(self.io_loop.run_sync(
    ...
    def test_add_thread_close_idempotent(self):
        loop = AddThreadSelectorEventLoop(
...
class AnyThreadEventLoopPolicyTest(unittest.TestCase):
    ...
    def test_asyncio_accessor(self):
        with warnings.catch_warnings():
            ...
            # loop.
            self.assertRaises(
                RuntimeError, self.executor.submit(
            ...
            self.executor.submit(
                lambda: asyncio.get_event_loop().close()).result()  # type: ignore
    ...
    def test_tornado_accessor(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", DeprecationWarning)
            self.assertIsInstance(self.executor.submit(
                IOLoop.current).result(), IOLoop)
            # Clean up to silence leak warnings. Always use asyncio since
            # IOLoop doesn't (currently) close the underlying loop.
            self.executor.submit(
                lambda: asyncio.get_event_loop().close()).result()  # type: ignore
