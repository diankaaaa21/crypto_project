...
class BadSSLOptionsTest(unittest.TestCase):
    ...
    def test_missing_key(self):
        ...
        HTTPServer(
            application,
            ssl_options={"certfile": existing_certificate,
...
class HTTPConnectionTest(AsyncHTTPTestCase):
    ...
    def raw_fetch(self, headers, body, newline=b"\r\n"):
        with closing(IOStream(socket.socket())) as stream:
            ...
            stream.write(
                newline.join(
                    headers + [utf8("Content-Length: %d" % len(body))])
                + newline
                + newline
                + body
            )
    ...
    def test_newlines(self):
        ...
        for newline in (b"\r\n", b"\n"):
            response = self.raw_fetch(
                [b"GET /hello HTTP/1.0"], b"", newline=newline)
...
class TypeCheckHandler(RequestHandler):
    def prepare(self):
        ...
        self.check_type("header_key", list(
            self.request.headers.keys())[0], str)
        self.check_type("header_value", list(
            self.request.headers.values())[0], str)

        self.check_type("cookie_key", list(
            self.request.cookies.keys())[0], str)
        self.check_type(
        ...
        self.check_type("arg_value", list(
            self.request.arguments.values())[0][0], bytes)
    ...
    def test_malformed_first_line_log(self):
        with ExpectLog(gen_log, ".*Malformed HTTP request line", level=logging.INFO):
            ...
            self.io_loop.add_timeout(
                datetime.timedelta(seconds=0.05), self.stop)
            self.wait()
...
class XHeaderTest(HandlerBaseTestCase):
    ...
    def test_ip_headers(self):
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=valid_ipv4_list)["remote_ip"], "4.4.4.4"
        )
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=invalid_chars)["remote_ip"], "127.0.0.1"
        )

        invalid_chars_list = {"X-Forwarded-For": "4.4.4.4, 5.5.5.5<script>"}
        self.assertEqual(
            self.fetch_json(
                "/", headers=invalid_chars_list)["remote_ip"], "127.0.0.1"
        )

        invalid_host = {"X-Real-IP": "www.google.com"}
        self.assertEqual(
            self.fetch_json(
                "/", headers=invalid_host)["remote_ip"], "127.0.0.1"
        )
    ...
    def test_scheme_headers(self):
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=https_scheme)["remote_protocol"], "https"
        )

        https_forwarded = {"X-Forwarded-Proto": "https"}
        self.assertEqual(
            self.fetch_json(
                "/", headers=https_forwarded)["remote_protocol"], "https"
        )

        https_multi_forwarded = {"X-Forwarded-Proto": "https , http"}
        self.assertEqual(
            self.fetch_json(
                "/", headers=https_multi_forwarded)["remote_protocol"],
            "http",
        )
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=http_multi_forwarded)["remote_protocol"],
            "https",
        )
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=bad_forwarded)["remote_protocol"], "http"
        )
...
class SSLXHeaderTest(AsyncHTTPSTestCase, HandlerBaseTestCase):
    ...
    def test_request_without_xprotocol(self):
        ...
        self.assertEqual(
            self.fetch_json(
                "/", headers=http_scheme)["remote_protocol"], "http"
        )

        bad_scheme = {"X-Scheme": "unknown"}
        self.assertEqual(
            self.fetch_json(
                "/", headers=bad_scheme)["remote_protocol"], "https"
        )
...
class InvalidOutputContentLengthTest(AsyncHTTPTestCase):
    class MessageDelegate(HTTPMessageDelegate):
        ...
        def headers_received(self, start_line, headers):
            ...
            self.connection.write_headers(
                ResponseStartLine("HTTP/1.1", 200, "OK"),
                HTTPHeaders(
                    {"Content-Length": content_lengths[headers["x-test"]]}),
            )
    ...
    def test_invalid_output_content_length(self):
        with self.subTest("normal"):
            response = self.fetch(
                "/", method="GET", headers={"x-test": "normal"})
            response.rethrow()
...
class MaxHeaderSizeTest(AsyncHTTPTestCase):
    ...
    def test_large_headers(self):
        with ExpectLog(gen_log, "Unsatisfiable read", required=False):
            try:
                self.fetch(
                    "/", headers={"X-Filler": "a" * 1000}, raise_error=True)
        ...
        class StreamingHandler(RequestHandler):
            ...
            def prepare(self):
                ...
                if "expected_size" in self.request.arguments:
                    conn.set_max_body_size(
                        int(self.get_argument("expected_size")))
                if "body_timeout" in self.request.arguments:
                    conn.set_body_timeout(
