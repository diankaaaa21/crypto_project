...
class TwitterClientHandler(RequestHandler, TwitterMixin):
    def initialize(self, test):
        ...
        self._OAUTH_AUTHORIZE_URL = test.get_url("/oauth1/server/authorize")
        self._OAUTH_AUTHENTICATE_URL = test.get_url(
            "/twitter/server/authenticate")
...
class AuthTest(AsyncHTTPTestCase):
    ...
    def test_oauth10a_redirect_error(self):
        with mock.patch.object(OAuth1ServerRequestTokenHandler, "get") as get:
            ...
            with ExpectLog(app_log, "Uncaught exception"):
                response = self.fetch(
                    "/oauth10a/client/login", follow_redirects=False)
    ...
    def test_twitter_authenticate_redirect(self):
        response = self.fetch(
            "/twitter/client/authenticate", follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(
            response.headers["Location"].endswith(
...
class GoogleOAuth2AuthorizeHandler(RequestHandler):
    def get(self):
        ...
        self.redirect(url_concat(self.get_argument(
