...
class Recompiler:
    ...
    def _generate(self, step_name):
        ...
        for name, (tp, quals) in sorted(lst):
            ...
            try:
                method = getattr(self, "_generate_cpy_%s_%s" %
                                 (kind, step_name))
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        def need_indirection(type):
            return isinstance(type, model.StructOrUnion) or (
                isinstance(
                    type, model.PrimitiveType) and type.is_complex_type()
            )
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                if fbitsize >= 0:
                    ...
                    isinstance(
                        fldtype, model.ArrayType) and fldtype.length is None
                ):
                    size = "(size_t)-1"
                else:
                    size = "sizeof(((%s)0)->%s)" % (
                        tp.get_c_name(
                            "*") if named_ptr is None else named_ptr.name,
                        fldname,
                    )
    ...
    def _generate_cpy_constant_ctx(self, tp, name):
        ...
        self._lsts["global"].append(GlobalExpr(
    ...
    def _generate_cpy_macro_ctx(self, tp, name):
        ...
        type_op = CffiOp(OP_CONSTANT_INT, -1)
        self._lsts["global"].append(
            GlobalExpr(name, "_cffi_const_%s" %
                       name, type_op, check_value=check_value)
        )
