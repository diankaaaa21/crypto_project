...
def _isUncanonicalNumber(s):
    ...
    if f not in _CANONICAL_NUMBERS:
        # Only the canonical numbers are required to be canonical.
        return False
    ...
    for char in text:
        if state == "startoflabelname":
            ...
            if char == "\\":
                ...
                if ln in labels:
                    raise ValueError(
                        "Invalid line, duplicate label name: " + text)
...
def _parse_sample(text):
    ...
    if separator not in text:
        ...
        labels, labels_len = _parse_labels_with_state_machine(
            text[label_start + 1:])
...
def _parse_remaining_text(text):
    ...
    for char in it:
        if state == "timestamp":
            ...
            if char == "{":
                ...
                exemplar_labels = _parse_labels(
                    text[label_start + 1: label_end])
    ...
    if exemplar_labels is not None:
        exemplar_length = sum(len(k) + len(v)
                              for k, v in exemplar_labels.items())
...
def _check_histogram(samples, name):
    ...
    def do_checks():
        ...
        if has_sum and count is None:
            raise ValueError(
                "_count must be present if _sum is present: " + name)
        if has_gsum and count is None:
            raise ValueError(
                "_gcount must be present if _gsum is present: " + name)
        if not (has_sum or has_gsum) and count is not None:
            raise ValueError(
                "_sum/_gsum must be present if _count is present: " + name)
    ...
    for line in fd:
        ...
        if line == "# EOF":
            ...
            if parts[1] == "HELP":
                ...
                allowed_names = [
                    name + n for n in type_suffixes.get(typ, [""])]
            elif parts[1] == "UNIT":
                if unit is not None:
            ...
            if name + "_bucket" == sample.name and (
                not isinstance(
                    sample.value, int) and not sample.value.is_integer()
            ):
            ...
            if typ == "info" and sample.value != 1:
                raise ValueError(
                    "Info samples can only have value one: " + line)
            ...
            if (
                ...
                raise ValueError(
                    "Counter-like samples cannot be negative: " + line)
