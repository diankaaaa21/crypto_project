...
def start_wsgi_server(
    ...
    httpd = make_server(addr, port, app, TmpServer,
            ...
            for s in metric.samples:
                for suffix in ["_created", "_gsum", "_gcount"]:
                    if s.name == metric.name + suffix:
                        ...
                        om_samples.setdefault(
                            suffix, []).append(sample_line(s))
                        break
                else:
...
def _make_handler(
    ...
    def handle() -> None:
        ...
        if resp.code >= 400:
            raise OSError(
                f"error talking to pushgateway: {resp.code} {resp.msg}")
...
def pushadd_to_gateway(
    ...
    _use_gateway("POST", gateway, job, registry,
                 grouping_key, timeout, handler)
