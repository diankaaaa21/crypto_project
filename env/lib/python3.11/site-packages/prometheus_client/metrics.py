...
class MetricWrapperBase(Collector):
    ...
    def _raise_if_not_observable(self):
        ...
        if not self._is_observable():
            raise ValueError("%s metric is missing label values" %
                             str(self._type))
    ...
    def collect(self) -> Iterable[Metric]:
        ...
        for suffix, labels, value, timestamp, exemplar in self._samples():
            metric.add_sample(self._name + suffix, labels,
    ...
    def __init__(
        ...
        self._name = _build_full_name(
            self._type, name, namespace, subsystem, unit)
    ...
    def labels(self: T, *labelvalues: Any, **labelkwargs: Any) -> T:
        ...
        if not self._labelnames:
            raise ValueError(
                "No label names were set when constructing %s" % self)
    ...
    def _child_samples(self) -> Iterable[Sample]:  # pragma: no cover
        raise NotImplementedError(
            "_child_samples() must be implemented by %r" % self)
    ...
    def _metric_init(self):  # pragma: no cover
        ...
        """
        raise NotImplementedError(
            "_metric_init() must be implemented by %r" % self)
...
class Enum(MetricWrapperBase):
    ...
    def _child_samples(self) -> Iterable[Sample]:
        with self._lock:
            return [
                Sample("", {self._name: s}, 1 if i ==
                       self._value else 0, None, None)
