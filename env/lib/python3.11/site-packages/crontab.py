...
class CronTab:
    ...
    def __setattr__(self, name, value):
        ...
        if name == "lines" and value:
            for line in value:
                self.append(CronItem.from_line(
                    line, cron=self), line, read=True)
    ...
    def read(self, filename=None):
        ...
        if self.intab is not None:
            ...
            try:
                process = Process(self.cron_command, l="",
                                  **self.user_opt).run()
    ...
    def write(self, filename=None, user=None, errors=False):
        ...
        if self.filen:
            fileh = open(
                self.filen, "wb")  # pylint: disable=consider-using-with
        else:
...
class CronSlice:
    ...
    def test_value(self, value, sunday=None):
        ...
        if int(value) < self.min or int(value) > self.max:
            raise ValueError(
                f"'{value}', not in {self.min}-{self.max} for {self.name}")
        return value
