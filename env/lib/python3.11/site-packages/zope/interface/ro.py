...
class C3:
    ...
    def __init__(self, C, memo):
        ...
        self.base_tree = (
            [[C]] + [memo[base].mro() for base in C.__bases__] +
            [list(C.__bases__)]
        )
    ...
    def _nonempty_bases_ignoring(base_tree, ignoring):
        return list(
            filter(
                None, [[b for b in bases if b is not ignoring]
                       for bases in base_tree]
            )
        )
...
class _ROComparison:
    ...
    class ReplacedBy:  # pragma: no cover
        ...
        def __iter__(self):
            lines = [self.prefix + str(item) +
                     self.suffix for item in self.chunk]
