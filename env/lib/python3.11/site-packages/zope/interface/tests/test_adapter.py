...
class BaseAdapterRegistryTests(unittest.TestCase):
    ...
    def test_lookup_delegation(self):
        ...
        for name in CUT._delegated:
            self.assertIs(getattr(registry, name),
                          getattr(registry._v_lookup, name))
    ...
    def test_register_multiple_allRegistrations(self):
        ...
        self.assertEqual(
            registry._adapters, build_adapters(
                L=CustomSequence, MT=CustomMapping)
        )
    ...
    def test_register_with_same_value(self):
        ...
        self.assertEqual(
            registered,
            # required  # provided  # name  # value
            [((Interface,), IR0, "", _value)],
        )
    ...
    def test_unsubscribe_hit(self):
        ...
        self.assertEqual(
            registry._subscribers[1], MT(
                {IB1: MT({None: MT({"": L((orig,))})})})
        )
    ...
    def test_subscribe_multiple_allRegistrations(self):
        ...
        self.assertEqual(registry._provided, build_provided(
            P=self._getProvidedType()))
        ...
        self.assertEqual(
            registry._subscribers,
            build_subscribers(L=CustomSequence,
                              F=CustomLeafSequence, MT=CustomMapping),
        )
...
class AdapterLookupBaseTests(unittest.TestCase):
    ...
    def test_ctor_w_registry_provided(self):
        ...
        self.assertEqual(
            sorted(alb._extendors[Interface]), sorted([IFoo, IBar]))
