    ...
    def __init__(
        ...
        def get_width() -> AnyDimension:
            if width is None:
                ...
                if text:
                    longest_line = max(get_cwidth(line)
        ...
        def _pageup(event: E) -> None:
            ...
            if w.render_info:
                self._selected_index = max(
                    0, self._selected_index -
                    len(w.render_info.displayed_lines)
                )
        ...
        def _find(event: E) -> None:
            ...
            for value in values[self._selected_index + 1:] + values:
                text = fragment_list_to_text(
                    to_formatted_text(value[1])).lower()
    ...
    def _get_text_fragments(self) -> StyleAndTextTuples:
        ...
        for i, value in enumerate(self.values):
            ...
            result.extend(to_formatted_text(
                value[1], style=self.default_style))
            result.append(("", "\n"))
                    ...
                    content=VSplit(
                        [
                            ...
                            Window(
                                ...
                                width=lambda: D(weight=int(
                                    100 - self._percentage)),
                            ),
                        ]
                    ),
                ),
