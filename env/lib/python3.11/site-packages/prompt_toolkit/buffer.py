...
class Buffer:
    ...
    def save_to_undo_stack(self, clear_redo_stack: bool = True) -> None:
        ...
        if self._undo_stack and self._undo_stack[-1][0] == self.text:
            self._undo_stack[-1] = (self._undo_stack[-1]
    ...
    def paste_clipboard_data(
        ...
        assert paste_mode in (PasteMode.VI_BEFORE,
                              PasteMode.VI_AFTER, PasteMode.EMACS)
                ...
                if new_index is not None:
                    ...
                    for i in range(working_index - 1, -2, -1):
                        ...
                        if new_index is not None:
                            return (
                                i,
                                Document(document.text, len(
                                    document.text) + new_index),
                            )
            return None
