...
class TextObject:
    ...
    def operator_range(self, document: Document) -> tuple[int, int]:
        ...
        if self.type == TextObjectType.LINEWISE:
            ...
            start = doc.translate_row_col_to_index(
                row, 0) - doc.cursor_position
            row, col = doc.translate_index_to_position(
        ...
        *keys: Keys | str,
        ...
        def decorator(text_object_func: _TOF) -> _TOF:
            ...
            def _apply_operator_to_text_object(event: E) -> None:
                ...
                event._arg = str((vi_state.operator_arg or 1)
                                 * (event.arg or 1))
...
def load_vi_bindings() -> KeyBindingsBase:
    ...
    def _end_of_screen(event: E) -> TextObject:
        ...
        if w and w.render_info:
            ...
            pos = (
                b.document.translate_row_col_to_index(
                    w.render_info.last_visible_line(
                        before_scroll_offset=True), 0
                )
                - b.cursor_position
            )
        ...
        if event._arg:
            ...
            return TextObject(
                d.translate_row_col_to_index(
                    event.arg - 1, 0) - d.cursor_position,
                type=TextObjectType.LINEWISE,
            )
        else:
...
def load_vi_search_bindings() -> KeyBindingsBase:
    ...
    handle(
        "/",
        filter=(vi_navigation_mode |
                vi_selection_mode) & ~vi_search_direction_reversed,
