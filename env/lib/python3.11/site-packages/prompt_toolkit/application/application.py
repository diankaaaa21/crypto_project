...
class Application(Generic[_AppResult]):
    ...
    def __init__(
        ...
        color_depth: ColorDepth | Callable[[],
                                           ColorDepth | None] | None = None,
...
class _CombinedRegistry(KeyBindingsBase):
    ...
    def _key_bindings(self) -> KeyBindingsBase:
        ...
        return self._cache.get(
            key, lambda: self._create_key_bindings(
                current_window, other_controls)
        )
...
def attach_winch_signal_handler(
    ...
    loop = get_running_loop()
    previous_winch_handler = getattr(
        loop, "_signal_handlers", {}).get(sigwinch)
