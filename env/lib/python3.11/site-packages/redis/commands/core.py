...
class ManagementCommands(CommandsProtocol):
        ...
        **kwargs,
        ...
        if _type is not None:
            ...
            if str(_type).lower() not in client_types:
                raise DataError(
                    f"CLIENT KILL type must be one of {client_types!r}")
    ...
    def client_list(
        ...
        if _type is not None:
            ...
            if str(_type).lower() not in client_types:
                raise DataError(
                    f"CLIENT LIST _type must be one of {client_types!r}")
            args.append(b"TYPE")
            args.append(_type)
...
class BasicKeyCommands(CommandsProtocol):
    ...
    def stralgo(
        ...
        value1: KeyT,
        value2: KeyT,
        specific_argument: Union[Literal["strings"],
        ...
        **kwargs,
        ...
        if algo not in supported_algo:
            ...
            raise DataError(
                f"The supported algorithms are: {supported_algos_str}")
        ...
        pieces: list[EncodableT] = [
            algo, specific_argument.upper(), value1, value2]
        if len:
            pieces.append(b"LEN")
        if idx:
            pieces.append(b"IDX")
...
class StreamCommands(CommandsProtocol):
    ...
    def xclaim(
        ...
        kwargs = {}
        pieces: list[EncodableT] = [
            name, groupname, consumername, str(min_idle_time)]
    ...
    def xreadgroup(
        ...
        if block is not None:
            if not isinstance(block, int) or block < 0:
                raise DataError(
                    "XREADGROUP block must be a non-negative integer")
            pieces.append(b"BLOCK")
            pieces.append(str(block))
        if noack:
...
class GeoCommands(CommandsProtocol):
    ...
    def geoadd(
        ...
        if len(values) % 3 != 0:
            raise DataError(
                "GEOADD requires places with lon, lat and name values")
        pieces = [name]
        if nx:
            pieces.append("NX")
        if xx:
    ...
    def _georadiusgeneric(
        ...
        if kwargs["store"] and kwargs["store_dist"]:
            raise DataError(
                "GEORADIUS store and store_dist cant be set together")
    ...
    def _geosearchgeneric(
        ...
        if kwargs["longitude"] is not None and kwargs["latitude"] is not None:
            pieces.extend(
                [b"FROMLONLAT", kwargs["longitude"], kwargs["latitude"]])

        # BYRADIUS or BYBOX
        if kwargs["radius"] is None:
            if kwargs["width"] is None or kwargs["height"] is None:
                raise DataError(
                    "GEOSEARCH must have radius or width and height")
        ...
        if kwargs["width"] and kwargs["height"]:
            pieces.extend([b"BYBOX", kwargs["width"],
                          kwargs["height"], kwargs["unit"]])
        ...
        if kwargs["count"]:
            ...
            raise DataError(
                "GEOSEARCH ``any`` can't be provided without count")
