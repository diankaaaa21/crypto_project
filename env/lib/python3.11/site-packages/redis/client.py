...
class Redis(RedisModuleCommands, CoreCommands, SentinelCommands):
    ...
    def from_url(cls, url: str, **kwargs) -> None:
        ...
        """
        single_connection_client = kwargs.pop(
...
class PubSub:
    ...
    def __init__(
        ...
        if self.encoder.decode_responses:
            ...
            self.health_check_response = [
                b"pong", self.health_check_response_b]
    ...
    def handle_message(self, response, ignore_subscribe_messages=False):
        ...
        if response is None:
            return None
        if isinstance(response, bytes):
            response = [b"pong", response] if response != b"PONG" else [
                b"pong", b""]
...
class Pipeline(Redis):
    ...
    def _execute_pipeline(self, connection, commands, raise_on_error):
        ...
        for args, options in commands:
            try:
                response.append(self.parse_response(
                    connection, args[0], **options))
            except ResponseError as e:
                response.append(e)
    ...
    def _disconnect_raise_reset(self, conn: Redis, error: Exception) -> None:
        ...
        if not (
            TimeoutError in conn.retry_on_error and isinstance(
                error, TimeoutError)
        ):
            self.reset()
            raise error
