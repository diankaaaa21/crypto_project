...
class AbstractRedisCluster:
    ...
    RESULT_CALLBACKS = dict_merge(
        list_keys_to_dict(
            ["PUBSUB NUMSUB", "PUBSUB SHARDNUMSUB"], parse_pubsub_numsub),
        list_keys_to_dict(
        ...
        list_keys_to_dict(
            ...
            lambda command, res: all(
                res.values()) if isinstance(res, dict) else res,
        ),
        list_keys_to_dict(
            ["DBSIZE", "WAIT"],
            lambda command, res: sum(
                res.values()) if isinstance(res, dict) else res,
        ),
        list_keys_to_dict(
            ["CLIENT UNBLOCK"], lambda command, res: 1 if sum(
                res.values()) > 0 else 0
        ),
        ...
        list_keys_to_dict(
            ["SCRIPT EXISTS"], lambda command, res: [
                all(k) for k in zip(*res.values())]
        ),
        list_keys_to_dict(["SCRIPT FLUSH"], lambda command,
                          res: all(res.values())),
    )
    ...
    def replace_default_node(self, target_node: "ClusterNode" = None) -> None:
        ...
        if target_node:
            ...
            primaries = [node for node in self.get_primaries()
                         if node != curr_node]
            if primaries:
                ...
                replicas = [node for node in self.get_replicas()
                            if node != curr_node]
                if replicas:
    ...
    def _execute_command(self, target_node, *args, **kwargs):
        ...
        while ttl > 0:
            ...
            try:
                ...
                response = redis_node.parse_response(
...
class LoadBalancer:
    ...
    def get_server_index(self, primary: str, list_size: int) -> int:
        server_index = self.primary_to_idx.setdefault(
            primary, self.start_index)
        # Update the index
    ...
    def execute_command(self, *args):
        ...
        if self.connection is None:
            ...
            if self.push_handler_func is not None and not HIREDIS_AVAILABLE:
                self.connection._parser.set_push_handler(
                    self.push_handler_func)
    ...
    def get_sharded_message(
        ...
        if message is None:
            ...
            if message["channel"] in self.pending_unsubscribe_shard_channels:
                self.pending_unsubscribe_shard_channels.remove(
                    message["channel"])
