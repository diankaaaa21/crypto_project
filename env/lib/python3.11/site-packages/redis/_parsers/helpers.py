...
def parse_xinfo_stream(response, **options):
    ...
    if not options.get("full", False):
        ...
        data["entries"] = {_id: pairs_to_dict(
            entry) for _id, entry in data["entries"]}
...
def parse_slowlog_get(response, **options):
    ...
    def parse_item(item):
        result = {"id": item[0], "start_time": int(
            item[1]), "duration": int(item[2])}
...
def _parse_node_line(line):
    ...
    node_id, addr, flags, master_id, ping, pong, epoch, connected = line.split(" ")[
        :8]
    addr = addr.split("@")[0]
    node_dict = {
        "node_id": node_id,
...
def parse_acl_getuser(response, **options):
    ...
    for command in data["commands"].split(" "):
        categories.append(
            command) if "@" in command else commands.append(command)
...
_RedisCallbacksRESP2 = {
    ...
    **string_keys_to_dict(
    ...
    "GEOPOS": lambda r: list(
        map(lambda ll: (float(ll[0]), float(ll[1]))
...
_RedisCallbacksRESP3 = {
    ...
    **string_keys_to_dict("XREAD XREADGROUP", parse_xread_resp3),
    "ACL LOG": lambda r: (
        [
            {str_if_bytes(key): str_if_bytes(value)
