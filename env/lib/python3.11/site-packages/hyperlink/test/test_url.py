...
class TestURL(HyperlinkTestCase):
    ...
    def test_click(self):
        ...
        # and path
        self.assertTrue(
            "//foobar" not in urlpath.click(
                "http://www.foo.com/foobar").to_text()
        )
    ...
    def test_asURI(self):
        ...
        self.assertEqual(actualURI, expectedURI, "%r != %r" %
    ...
    def test_asIRI(self):
        ...
        self.assertEqual(actualIRI, expectedIRI, "%r != %r" %
                         (actualIRI, expectedIRI))
    ...
    def test_netloc_slashes(self):
        ...
        self.assertEqual(
            url.to_text(), "git+ftp://gitstub.biz/glyph/lefkowitz")
    ...
    def test_encoded_userinfo(self):
        ...
        iri = url.to_iri()
        assert iri.to_text(
            with_password=True) == "http://us her:pass@example.com"
    ...
    def test_normalize(self):
        ...
        assert (
            URL(path=("", "%te%sts")).normalize(
                percents=False).to_text() == "/%te%sts"
        )
        assert URL(path=("", "%te%sts")).normalize(
        ).to_text() == "/%25te%25sts"
        ...
        assert percenty_url.to_text(
            with_password=True) == "ftp://%:%@/%%%/%a%b?%=%%#%"
        assert (
    ...
    def test_idna_corners(self):
        ...
        assert URL.from_text(text).to_uri(
        ).get_decoded_url().host == "example.com"
