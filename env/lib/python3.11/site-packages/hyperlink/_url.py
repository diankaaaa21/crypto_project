...
class URL(object):
    ...
    def from_text(cls, text):
        ...
        if gs["query"]:
            query = tuple(
                (qe.split("=", 1) if "=" in qe else (
                    qe, None))  # type: ignore[misc]
            ...
            query=tuple(
                [
                    (
                        ...
                        _encode_query_value(
                            v, maximal=True) if v is not None else None,
                    )
    ...
    def set(self, name, value=None):
        ...
        idx = next((i for (i, (k, v)) in enumerate(
            self.query) if k == name), -1)
    ...
    def remove(
        ...
        if limit is None:
            if value is _UNSET:
                ...
                nq = [(k, v)
                      for (k, v) in self.query if not (k == name and v == value)]
        else:
            nq, removed_count = [], 0
...
class DecodedURL(object):
    ...
    def normalize(
        ...
        """
        return self._clone(
            self._url.normalize(scheme, host, path, query,
                                fragment, userinfo, percents)
        )
