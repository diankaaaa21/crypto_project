...
def _get_distutils_version(
    dist, keyword, value
):  # type: (_Distribution, object, object) -> None
    """
    ...
    for item in sp_command.find_all_modules():
        if item[1] == "_version":
            version_path = os.path.join(
                os.path.dirname(item[2]), "_version.py")
...
"""
    ...
    if not isinstance(package, str):
        raise TypeError(
            "The project name must be a string, but found {}".format(
                type(package))
        )
...
def _extract_tool_incremental(
    data,
):  # type: (Dict[str, object]) -> Optional[Dict[str, object]]
    if "tool" not in data:
        return None
