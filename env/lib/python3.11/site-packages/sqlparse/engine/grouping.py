...
def group_typed_literal(tlist):
    ...
    _group(tlist, sql.TypedLiteral, match,
...
def group_comparison(tlist):
    ...
    valid_prev = valid_next = valid
    _group(tlist, sql.Comparison, match, valid_prev,
           valid_next, post, extend=False)
...
def group_operator(tlist):
    ...
    def valid(token):
        return imt(token, i=sqlcls, t=ttypes) or (
            token
            and token.match(
                T.Keyword, ("CURRENT_DATE", "CURRENT_TIME",
                            "CURRENT_TIMESTAMP")
            )
        )
    ...
    valid_prev = valid_next = valid
    _group(tlist, sql.Operation, match, valid_prev,
           valid_next, post, extend=False)
