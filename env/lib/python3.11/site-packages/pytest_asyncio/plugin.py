...
from collections.abc import Coroutine as AbstractCoroutine
...
def pytest_report_header(config: Config) -> list[str]:
    ...
    default_fixture_loop_scope = config.getini(
        "asyncio_default_fixture_loop_scope")
    ...
    def _asyncgen_fixture_wrapper(request: FixtureRequest, **kwargs: Any):
        ...
        def finalizer() -> None:
            ...
            task = _create_task_in_context(
                event_loop, async_finalizer(), context)
...
def _get_event_loop_fixture_id_for_async_fixture(
    ...
    default_loop_scope = request.config.getini(
        "asyncio_default_fixture_loop_scope")
    loop_scope = (
...
class PytestAsyncioFunction(Function):
    ...
    def _from_function(cls, function: Function, /) -> Function:
        ...
        subclassed_function_signature = inspect.signature(
...
class AsyncGenerator(PytestAsyncioFunction):
    ...
    def _from_function(cls, function: Function, /) -> Function:
        ...
        async_gen_item.warn(PytestCollectionWarning(
            unsupported_item_type_message))
...
def pytest_pycollect_makeitem_convert_async_functions_to_subclass(
    ...
    try:
        node_or_list_of_nodes: (
            pytest.Item | pytest.Collector | list[pytest.Item |
...
def _get_marked_loop_scope(
    ...
    if asyncio_marker.args or (
        asyncio_marker.kwargs and set(
            asyncio_marker.kwargs) - {"loop_scope", "scope"}
    ):
        raise ValueError(
            "mark.asyncio accepts only a keyword argument 'loop_scope'.")
    ...
    try:
        ...
        if not loop.is_closed():
            try:
                ...
                warnings.warn(
                    f"Error cleaning up asyncio loop: {e}", RuntimeWarning)
            finally:
                loop.close()
