    ...
    if not years and days < 1:
        if delta.seconds == 0:
            if min_unit == Unit.MICROSECONDS and delta.microseconds < 1000:
                return (
                    _ngettext("%d microsecond", "%d microseconds",
                              delta.microseconds)
                    % delta.microseconds
                )
            ...
            if min_unit == Unit.MILLISECONDS or (
                ...
                return (
                    _ngettext("%d millisecond", "%d milliseconds",
                              int(milliseconds))
                    % milliseconds
                )
            return _("a moment")
...
def precisedelta(
    ...
    years, days = _quotient_and_remainder(
        days, 365, YEARS, min_unit, suppress_set)
    months, days = _quotient_and_remainder(
        days, 30.5, MONTHS, min_unit, suppress_set)
    ...
    hours, secs = _quotient_and_remainder(
        secs, 3600, HOURS, min_unit, suppress_set)
    minutes, secs = _quotient_and_remainder(
        secs, 60, MINUTES, min_unit, suppress_set)
