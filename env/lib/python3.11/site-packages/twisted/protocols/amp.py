...
from twisted.python._tzhelper import FixedOffsetTimeZone as _FixedOffsetTZInfo
...
class AmpBox(Dict[bytes, bytes]):
    ...
    def serialize(self):
        ...
        for k, v in i:
            ...
            if type(v) == str:
                raise TypeError(
                    f"Unicode value for key {k!r} not allowed: {v!r}")
...
class AmpList(Argument):
    ...
    def toStringProto(self, inObject, proto):
        return b"".join(
            [
                _objectsToStrings(objects, self.subargs,
                                  Box(), proto).serialize()
...
class _CommandMeta(type):
    ...
    def __new__(
        ...
        newtype: Type[Command] = type.__new__(
            cls, name, bases, attrs)  # type:ignore
        ...
        for bname, _ in newtype.arguments:
            if not isinstance(bname, bytes):
                raise TypeError(
                    f"Argument names must be byte strings, got: {bname!r}")
        for bname, _ in newtype.response:
            if not isinstance(bname, bytes):
                raise TypeError(
                    f"Response names must be byte strings, got: {bname!r}")
        ...
        for _, bname in newtype.errors.items():
            if not isinstance(bname, bytes):
                raise TypeError(
                    f"Error names must be byte strings, got: {bname!r}")
...
class Command(metaclass=_CommandMeta):
    ...
    def makeArguments(cls, objects, proto):
        ...
        for intendedArg in objects:
            if intendedArg not in allowedNames:
                raise InvalidSignature(
                    f"{intendedArg} is not a valid argument")
    ...
    def _doCommand(self, proto):
        ...
        def _massageError(error):
            ...
            rje = error.value
            errorType = self.reverseErrors.get(
    ...
    def connectionLost(self, reason):
        ...
        if self.innerProtocol is not None:
            ...
            if self.innerProtocolClientFactory is not None:
                self.innerProtocolClientFactory.clientConnectionLost(
                    None, reason)
