...
class RegisterProxy(Proxy):
    ...
    def register(self, message, host, port):
        ...
        if message.headers.get("expires", [None])[0] == "0":
            ...
            if contact is not None:
                name, contactURL, params = parseAddress(
                    contact, host=host, port=port)
                d = self.registry.registerAddress(
                    message.uri, toURL, contactURL)
            else:
    ...
    def unregister(self, message, toURL, contact):
        try:
            ...
            if expires == 0:
                ...
                d = self.registry.unregisterAddress(
                    message.uri, toURL, contactURL)
    ...
    def _cbUnregister(self, registration, message):
        ...
        msg.headers.setdefault("contact", []).append(
            registration.contactURL.toString())
    ...
    def registerAddress(self, domainURL, logicalURL, physicalURL):
        ...
        if logicalURL.username in self.users:
            ...
            dc = reactor.callLater(
                3600, self._expireRegistration, logicalURL.username)
        log.msg(
            f"Registered {logicalURL.toString()} at {physicalURL.toString()}")
