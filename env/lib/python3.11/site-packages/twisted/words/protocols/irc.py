...
class IRCClient(basic.LineReceiver):
    ...
    def _sendLine(self):
        if self._queue:
            ...
            self._queueEmptying = reactor.callLater(
                self.lineRate, self._sendLine)
        else:
    ...
    def dccResume(self, user, fileName, port, resumePos):
        ...
        """
        self.ctcpMakeQuery(
            user, [("DCC", ["RESUME", fileName, port, resumePos])])
    ...
    def dccAcceptResume(self, user, fileName, port, resumePos):
        ...
        """
        self.ctcpMakeQuery(
            user, [("DCC", ["ACCEPT", fileName, port, resumePos])])
    ...
    def ctcpQuery_FINGER(self, user, channel, data):
        if data is not None:
            self.quirkyMessage(
                f"Why did {user} send '{data}' with a FINGER query?")
    ...
    def ctcpQuery_VERSION(self, user, channel, data):
        if data is not None:
            self.quirkyMessage(
                f"Why did {user} send '{data}' with a VERSION query?")
    ...
    def ctcpQuery_SOURCE(self, user, channel, data):
        if data is not None:
            self.quirkyMessage(
                f"Why did {user} send '{data}' with a SOURCE query?")
        if self.sourceURL:
            nick = user.split("!")[0]
            ...
            self.ctcpMakeReply(
                nick, [("SOURCE", self.sourceURL), ("SOURCE", None)])

    def ctcpQuery_USERINFO(self, user, channel, data):
        if data is not None:
            self.quirkyMessage(
                f"Why did {user} send '{data}' with a USERINFO query?")
        if self.userinfo:
            nick = user.split("!")[0]
    ...
    def ctcpQuery_CLIENTINFO(self, user, channel, data):
        ...
        if not data:
            ...
            # methods.
            names = sorted(reflect.prefixedMethodNames(
                self.__class__, "ctcpQuery_"))
    ...
    def ctcpQuery_ERRMSG(self, user, channel, data):
        ...
        nick = user.split("!")[0]
        self.ctcpMakeReply(
            nick, [("ERRMSG", "%s :No error has occurred." % data)])

    def ctcpQuery_TIME(self, user, channel, data):
        if data is not None:
            self.quirkyMessage(
                f"Why did {user} send '{data}' with a TIME query?")
        nick = user.split("!")[0]
        self.ctcpMakeReply(
...
class DccFileReceive(DccFileReceiveBasic):
    ...
    def set_directory(self, directory):
        ...
        if not path.exists(directory):
            raise OSError(
                errno.ENOENT, "You see no directory there.", directory)
...
class _CharacterAttributes(_textattributes.CharacterAttributesMixin):
    ...
    attrs = {"bold": _BOLD, "reverseVideo": _REVERSE_VIDEO,
