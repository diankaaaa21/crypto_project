    ...
    def sendMessage(self, text, meta=None):
        ...
        for line in text.split("\n"):
            if meta and meta.get("style", None) == "emote":
                self.account.client.ctcpMakeQuery(
...
class IRCGroup(basesupport.AbstractGroup):
    ...
    def imtarget_kick(self, target):
        ...
        self.account.client.sendLine(
            f"KICK #{self.name} {target.name} :{reason}")
...
class IRCProto(basesupport.AbstractClientMixin, irc.IRCClient):
    def __init__(self, account, chatui, logonDeferred=None):
        basesupport.AbstractClientMixin.__init__(
            self, account, chatui, logonDeferred)
        self._namreplies = {}
        self._ingroups = {}
        self._groups = {}
        self._topics = {}
    ...
    def action(self, username, channel, emote):
        ...
        if channel[0] == "#":
            ...
            self.getGroupConversation(
                group).showGroupMessage(username, emote, meta)
            return
        self.chat.getConversation(self.getPerson(
            username)).showMessage(emote, meta)
    ...
    def irc_333(self, prefix, params):
        group = params[1][1:]
        self.getGroupConversation(group).setTopic(
            self._topics[group], params[2])
    ...
    def irc_NICK(self, prefix, params):
        ...
        for group in self._ingroups[fromNick]:
            self.getGroupConversation(
                group).memberChangedNick(fromNick, toNick)
...
class IRCAccount(basesupport.AbstractAccount):
    ...
    def _startLogOn(self, chatui):
        ...
        cc = protocol.ClientCreator(
            reactor, IRCProto, self, chatui, logonDeferred)
