...
class IRCUser(irc.IRC):
    ...
    def logInAs(self, nickname, password):
        d = self.factory.portal.login(
            credentials.UsernamePassword(
                nickname, password), self, iwords.IUser
        )
    ...
    def irc_MODE(self, prefix, params):
        ...
        *( ( "+" / "-" ) *( "i" / "w" / "o" / "O" / "r" ) )
        ...
        if channelOrUser.startswith("#"):
            ...
            d.addCallbacks(self._channelMode, ebGroup,
            ...
            d.addCallbacks(self._userMode, ebUser,
                           callbackArgs=tuple(params[1:]))
    ...
    def irc_PRIVMSG(self, prefix, params):
        ...
        def ebTarget(err):
            self.sendMessage(irc.ERR_NOSUCHNICK, targetName,
    ...
    def irc_NAMES(self, prefix, params):
        ...
        def cbGroup(group):
            self.names(
                self.name, "#" +
                group.name, [user.name for user in group.iterusers()]
            )
    ...
    def irc_LIST(self, prefix, params):
        ...
        def cbGroups(groups):
            ...
            d.addCallback(lambda results: self.list(
                [r for (s, r) in results if s]))
            return d
