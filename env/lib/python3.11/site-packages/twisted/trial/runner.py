...
class TrialSuite(TestSuite):
    ...
    def _bail(self):
        ...
        reactor.addSystemEventTrigger(
            "after", "shutdown", lambda: d.callback(None))
            ...
            try:
                ...
                if remaining == "":
                    raise reflect.ModuleNotFound(
                        f"The module {name} does not exist.")
...
class TrialRunner:
    ...
    reporterFactory: Callable[[TextIO, str, bool,
                               log.LogPublisher], itrial.IReporter]
    ...
    def _runWithoutDecoration(
        ...
        if self.mode == self.DRY_RUN:
            ...
            if self.mode == self.DEBUG:
                ...
                def run(): return self.debugger.runcall(suite.run, result)
            else:
                def run(): return suite.run(result)
