    ...
    def test_spawnProcess(self):
        ...
        for count in [1, 2]:
            self.reactor.spawnProcess(
                proto, sys.executable, args=[sys.executable])
...
class WorkerPoolTests(TestCase):
    ...
    def test_createLocalWorkers(self):
        ...
        workers = self.pool._createLocalWorkers(
            protocols, FilePath("path"), StringIO())
        for s in workers:
...
class LocalWorkerPool:
    ...
    async def start(
        ...
        workers = [self._workerFactory()
                   for i in range(self._config.numWorkers)]
