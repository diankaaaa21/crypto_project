        ...
        for n, (succeeded, failure) in enumerate(results):
            if not succeeded:
                self._logger.failure(
                    f"joining disttrial worker #{n} failed", failure)
...
class WorkerPool:
    ...
    def _launchWorkerProcesses(self, spawner, protocols, arguments):
        ...
        for worker in protocols:
            ...
            spawner(worker, sys.executable, args=args,
                    childFDs=childFDs, env=environ)
...
class DistTrialRunner:
    ...
    _reactor: IDistTrialReactor = field(
        ...
        converter=default_if_none(
            factory=_defaultReactor),  # type: ignore [misc]
    )
    # mypy doesn't understand the converter
    stream: TextIO = field(default=None, converter=default_if_none(
        sys.stdout))  # type: ignore [misc]
    ...
    async def runAsync(
        ...
        async def runAndReport(n: int) -> DistReporter:
            ...
            if self._exitFirst:
                ...
                # seen success.
                def casesCondition(_): return result.original.wasSuccessful()
            else:
                def casesCondition(_): return True
    ...
    def _run(self, test: Union[TestCase, TestSuite], untilFailure: bool) -> IReporter:
        ...
        self._reactor.addSystemEventTrigger(
            "before", "shutdown", maybeStopTests)
        ...
        assert isinstance(
            result, DistReporter), f"{result} is not DistReporter"
