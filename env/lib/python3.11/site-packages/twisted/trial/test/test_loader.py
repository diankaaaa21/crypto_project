...
class FileTests(packages.SysPathManglingTest):
    ...
    def test_directoryNotPackage(self) -> None:
        ...
        err = self.assertRaises(
            ValueError, runner.filenameToModule, emptyDir.path)
        self.assertEqual(
            str(err), f"{emptyDir.path!r} is not a package directory")
...
class LoaderTests(packages.SysPathManglingTest):
    ...
    def test_sortCases(self) -> None:
        ...
        # type: ignore[arg-type, union-attr, call-arg]
        self.loader.sorter = lambda x: sortDict.get(x.shortDescription(), -1)
        suite = self.loader.loadClass(sample.AlphabetTest)
        self.assertEqual(
            newOrder, [test._testMethodName for test in suite._tests])
    ...
    def test_loadNonClass(self) -> None:
        ...
        self.assertRaises(TypeError, self.loader.loadClass,
    ...
    def test_loadNonModule(self) -> None:
        ...
        self.assertRaises(TypeError, self.loader.loadModule,
                          sample.FooTest.test_foo)
    ...
    def test_loadNonPackage(self) -> None:
        ...
        self.assertRaises(TypeError, self.loader.loadPackage,
                          sample.FooTest.test_foo)
    ...
    def test_customLoadRaisesAttributeError(self) -> None:
        ...
        mockcustomsuite2.testSuite, original = (
            testSuite, mockcustomsuite2.testSuite)
        try:
            self.assertRaises(
                AttributeError, self.loader.loadModule, mockcustomsuite2)
        finally:
            mockcustomsuite2.testSuite = original
