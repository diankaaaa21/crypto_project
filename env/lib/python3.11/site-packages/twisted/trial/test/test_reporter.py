...
from unittest import TestSuite as PyUnitTestSuite
...
class ErrorReportingTests(StringTest):
    ...
    def test_formatErroredMethod(self):
        ...
        match = [
            ...
            re.compile(
                r"^\s+raise FoolishError." r".I am a broken setUp method..$"),
            (
    ...
    def test_hiddenException(self) -> None:
        ...
        assert_that(
            result, matches_result(errors=has_length(1),
        ...
        assert_that(
            ...
            isFailure(
                ...
                # type: ignore[arg-type]
                frames=has_item(similarFrame(
                    "testHiddenException", "erroneous.py")),
            ),
        )
...
class TracebackHandlingTests(unittest.SynchronousTestCase):
    ...
    def test_basic(self):
        ...
        self.checkFrames(
            frames, [("test_fail", "twisted/trial/test/erroneous")])
    ...
    def test_exception(self):
        ...
        frames = self.getErrorFrames(test)
        self.checkFrames(
            frames, [("test_exception", "twisted/trial/test/erroneous")])
...
class DirtyReactorTests(unittest.SynchronousTestCase):
    ...
    def test_dealsWithThreeTuples(self):
        ...
        result = UncleanWarningsReporterWrapper(
            reporter.Reporter(stream=self.output))
        result.addError(self.test, (self.dirtyError.type,
                        self.dirtyError.value, None))
...
class TrialNamesTests(unittest.SynchronousTestCase):
    ...
    def test_treeReporterWithDocstrings(self):
        ...
        self.assertEqual(result.getDescription(
            self), "test_treeReporterWithDocstrings")
...
class TodoTests(unittest.SynchronousTestCase):
    ...
    def test_exceptionTodo(self):
        ...
        try:
            ...
            error = e
        self.result.addExpectedFailure(
            self.test, Failure(error), makeTodo("todo!"))
        self.result.done()
...
class TreeReporterTests(unittest.SynchronousTestCase):
    ...
    def test_getPrelude(self):
        ...
        """
        self.assertEqual(
            ["foo.bar", "baz"], self.result._getPreludeSegments(
                "foo.bar.baz.qux")
        )
        self.assertEqual(
            ["foo", "bar"], self.result._getPreludeSegments("foo.bar.baz"))
...
class ExitWrapperTests(unittest.SynchronousTestCase):
    ...
    def test_doesNotStopOnUnexpectedSuccess(self):
        ...
        self.assertFalse(self.wrapped.shouldStop)
        self.assertEqual(self.result.unexpectedSuccesses,
                         [(self.test, self.failure)])
