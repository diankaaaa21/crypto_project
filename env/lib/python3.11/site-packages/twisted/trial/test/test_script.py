...
class LogfileTests(unittest.SynchronousTestCase):
    ...
    def test_default(self, workingDirectory: str) -> None:
        ...
        logPath: IFilePath = FilePath(
            workingDirectory).preauthChild(config["logfile"])
    ...
    def test_relativePath(self, logfile: str) -> None:
        ...
        logPath: IFilePath = FilePath(
            config["temp-directory"]).preauthChild(logfile)
        assert_that(logPath, fileContents(contains_string("something")))
...
class TestModuleTests(unittest.SynchronousTestCase):
    ...
    def test_testmoduleOnModuleName(self) -> None:
        ...
        try:
            ...
            self.assertEqual(0, len(self.config["tests"]))
            self.assertEqual(
                f"File {moduleName!r} doesn't exist\n", buffy.getvalue())
        finally:
            sys.stderr = stderr
    ...
    def test_parseLocalVariable(self) -> None:
        ...
        self.assertEqual(
            {"test-case-name": "twisted.trial.test.test_tests"}, localVars)
    ...
    def test_invalidDeclaration(self) -> None:
        self.assertRaises(
            ValueError, trial._parseLocalVariables, "-*- foo -*-")
        self.assertRaises(
    ...
    def test_variablesFromFile(self) -> None:
        ...
        self.assertEqual(
            {"test-case-name": "twisted.trial.test.test_log"}, localVars)
...
class OptionsTests(unittest.TestCase):
    ...
    def test_jobsConflictWithDebug(self) -> None:
        ...
        self.assertEqual(
            "You can't specify --debug when using --jobs", str(error))
    ...
    def test_jobsConflictWithProfile(self) -> None:
        ...
        self.assertEqual(
            "You can't specify --profile when using --jobs", str(error))
    ...
    def test_jobsConflictWithDebugStackTraces(self) -> None:
        ...
        self.assertEqual(
            "You can't specify --debug-stacktraces when using --jobs", str(
                error)
        )
    ...
    def test_orderConflictWithRandom(self) -> None:
        ...
        self.assertEqual(
            "You can't specify --random when using --order", str(error))
...
class MakeRunnerTests(unittest.TestCase):
    ...
    def test_jobsNotANumber(self) -> None:
        ...
        self.assertEqual(
            "Expecting integer argument to jobs, got 'nan'", str(exc))
    ...
    def test_jobsNonPositive(self) -> None:
        ...
        """
        exc = self.assertRaises(
            UsageError, self.options.parseOptions, ["--jobs", "0"])
        self.assertEqual(
            "Argument to jobs must be a strictly positive integer or 'auto'", str(
                exc)
        )
