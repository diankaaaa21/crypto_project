...
class StrictTodoMixin(ResultsTestMixin):
    ...
    def test_expectedFailures(self):
        ...
        reasonsGotten = [r.reason for t, e,
                         r in self.reporter.expectedFailures]
...
class GarbageCollectionTests(GCMixin, unittest.SynchronousTestCase):
    ...
    def test_collectCalled(self):
        ...
        test.run(result)
        self.assertEqual(
            self._collectCalled, ["collect", "setUp",
                                  "test", "tearDown", "collect"]
        )
...
class AddCleanupMixin:
    ...
    def test_addCleanupCalledInReverseOrder(self):
        ...
        self.test.run(self.result)
        self.assertEqual(["setUp", "runTest", "bar",
                         "foo", "tearDown"], self.test.log)
    ...
    def test_cleanupsContinueRunningAfterError(self):
        ...
        self.test.run(self.result)
        self.assertEqual(["setUp", "runTest", "foo",
                         "tearDown"], self.test.log)
        self.assertEqual(1, len(self.result.errors))
        [(test, error)] = self.result.errors
        self.assertEqual(test, self.test)
...
class TestDecoratorMixin:
    ...
    def assertTestsEqual(self, observed, expected):
        ...
        """
        self.assertEqual(observed.__class__,
                         expected.__class__, "Different class")
    ...
    def assertSuitesEqual(self, observed, expected):
        ...
        self.assertEqual(observed.__class__,
                         expected.__class__, "Different class")
        self.assertEqual(
            len(observed._tests), len(
                expected._tests), "Different number of tests."
        )
    ...
    def test_decorateDecoratedSuite(self):
        ...
        test = self.TestCase()
        decoratedTest = unittest.decorate(test, unittest.TestDecorator)
        redecoratedTest = unittest.decorate(
            decoratedTest, unittest.TestDecorator)
        self.assertTestsEqual(
            redecoratedTest, unittest.TestDecorator(decoratedTest))
    ...
    def test_decoratePreservesSuite(self):
        ...
        self.assertSuitesEqual(
            decorated, runner.DestructiveTestSuite(
                [unittest.TestDecorator(test)])
        )
