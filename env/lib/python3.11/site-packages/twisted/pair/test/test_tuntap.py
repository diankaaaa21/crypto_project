...
class TunnelDeviceTestsMixin:
    ...
    def setUp(self):
        ...
        self.fileno = self.system.open(
            b"/dev/net/tun", os.O_RDWR | os.O_NONBLOCK)
        self.addCleanup(self.system.close, self.fileno)

        mode = self.helper.TUNNEL_TYPE
        config = struct.pack("%dsH" % (_IFNAMSIZ,),
                             self._TUNNEL_DEVICE, mode.value)
...
setattr(
    FakeTunDeviceTests,
    "helper",
    TunHelper(FakeTunDeviceTests._TUNNEL_REMOTE,
              FakeTunDeviceTests._TUNNEL_LOCAL),
)
...
class TunnelTestsMixin:
    ...
    def test_unhandledEnvironmentReadError(self):
        ...
        """
        self.assertRaises(
            IOError, self._stopsReadingTest, IOError(
    ...
    def test_writeSequence(self):
        ...
        self.assertEqual(
            self.system.getTunnel(self.port).writeBuffer, deque(
                [b"".join(datagram)])
        )
    ...
    def test_listeningString(self):
        ...
        """
        self.port.startListening()
        self.assertRegex(str(self.port), fullyQualifiedName(
