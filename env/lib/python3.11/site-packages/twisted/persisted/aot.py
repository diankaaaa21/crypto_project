...
class Ref:
    ...
    def setRef(self, num):
        if self.refnum:
            raise ValueError(
                f"Error setting id {num}, I already have {self.refnum}")
        self.refnum = num

    def setObj(self, obj):
        if self.obj:
            raise ValueError(
                f"Error setting obj {obj}, I already have {self.obj}")
        self.obj = obj
...
def prettify(obj):
    if hasattr(obj, "getSource"):
        ...
        if t in _SIMPLE_BUILTINS:
            ...
            raise TypeError(
                f"Unsupported type {t} when trying to prettify {obj}.")
...
def unjellyFromSource(stringOrFile):
    ...
    if "app" in ns:
        ...
        raise ValueError(
            "%s needs to define an 'app', it didn't!" % stringOrFile)
    ...
    def jellyToAO(self, obj):
        ...
        if objType in _SIMPLE_BUILTINS:
            ...
            def _stateFrom(state):
                retval.setObj(
                    Instance(reflect.qual(obj.__class__),
            ...
            if objType is list:
                ...
                retval.setObj(
                    Copyreg(reflect.fullFuncName(
                        unpickleFunc), self.jellyToAO(state))
                )
