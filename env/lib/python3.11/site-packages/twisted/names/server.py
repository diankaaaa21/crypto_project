...
class DNSServerFactory(protocol.ServerFactory):
    ...
    def gotResolverResponse(self, response, protocol, message, address):
        ...
        self._verboseLog("Lookup found %d record%s" %
                         (l, l != 1 and "s" or ""))
    ...
    def handleOther(self, message, protocol, address):
        ...
        message.rCode = dns.ENOTIMP
        self.sendReply(protocol, message, address)
        self._verboseLog("Unknown op code (%d) from %r" %
                         (message.opCode, address))
    ...
    def messageReceived(self, message, proto, address=None):
        ...
        if self.verbose:
            if self.verbose > 1:
                ...
                s = " ".join(
                    [dns.QUERY_TYPES.get(q.type, "UNKNOWN")
                     for q in message.queries]
                )
            if not len(s):
                log.msg(
                    f"Empty query from {address or proto.transport.getPeer()!r}")
            else:
                log.msg(
                    f"{s} query from {address or proto.transport.getPeer()!r}")
