...
from twisted.names import cache, common, dns
from twisted.names import hosts as hostsModule
from twisted.names import resolve, root
...
class Resolver(common.ResolverBase):
    ...
    def _connectedProtocol(self, interface=""):
        ...
        while True:
            try:
                self._reactor.listenUDP(
                    dns.randomSource(), proto, interface=interface)
    ...
    def messageReceived(self, message, protocol, address=None):
        log.msg("Unexpected message (%d) received from %r" %
                (message.id, address))
    ...
    def _reissue(self, reason, addressesLeft, addressesUsed, query, timeout):
        ...
        d.addErrback(self._reissue, addressesLeft,
                     addressesUsed, query, timeout)
        return d
