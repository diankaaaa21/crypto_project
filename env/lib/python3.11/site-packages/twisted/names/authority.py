        ...
        if domain_records:
            for record in domain_records:
                ...
                if record.TYPE == dns.NS and name.lower() != self.soa[0].lower():
                    ...
                    authority.append(
                        dns.RRHeader(name, record.TYPE, dns.IN,
                                     ttl, record, auth=False)
                    )
        ...
        if self.soa[0].lower() == name.lower():
            ...
            for k, r in self.records.items():
                for rec in r:
                    ...
                    if rec.TYPE != dns.SOA:
                        results.append(
                            dns.RRHeader(k, rec.TYPE, dns.IN,
                                         ttl, rec, auth=True)
                        )
...
class BindAuthority(FileAuthority):
    ...
    def parseRecordLine(self, origin, ttl, line):
        ...
        """
        queryClasses = {qc.encode("ascii")
                        for qc in dns.QUERY_CLASSES.values()}
        ...
        self.addRecord(owner, ttl, nativeString(type),
                       domain, nativeString(cls), rdata)
