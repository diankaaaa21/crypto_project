...
class RoundtripDNSTests(unittest.TestCase):
    ...
    def test_MINFO(self):
        ...
        """
        self._recordRoundtripTest(
            dns.Record_MINFO(rmailbx=b"foo", emailbx=b"bar"))
...
class MessageComparisonTests(ComparisonTestsMixin, unittest.SynchronousTestCase):
    ...
    def test_authority(self):
        ...
        self.assertNormalEqualityImplementation(
            ...
            self.messageFactory(
                authority=[
                    dns.RRHeader(b"example.org", type=dns.SOA,
                                 payload=dns.Record_SOA())
                ]
            ),
        )
...
class DatagramProtocolTests(unittest.TestCase):
    ...
    def test_receiveMessageNotInLiveMessages(self):
        ...
        self.assertEqual(
            self.controller.messages[-1][0].toStr(), message.toStr())
...
class EqualityTests(ComparisonTestsMixin, unittest.TestCase):
    ...
    def test_name(self):
        ...
        same name value.
        """
        self._equalityTest(dns.Name(b"abc"), dns.Name(
            b"abc"), dns.Name(b"def"))
    ...
    def _simpleEqualityTest(self, cls):
        ...
        # Vary the TTL
        self._equalityTest(
            cls(b"example.com", 123), cls(
                b"example.com", 123), cls(b"example.com", 321)
        )
        # Vary the name
        self._equalityTest(
            cls(b"example.com", 123), cls(
                b"example.com", 123), cls(b"example.org", 123)
        )
    ...
    def test_rrheader(self):
        ...
        self._equalityTest(
            dns.RRHeader(b"example.com", cls=dns.IN,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", cls=dns.IN,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", cls=dns.CS,
                         payload=dns.Record_A("1.2.3.4")),
        )

        # Vary the ttl
        self._equalityTest(
            dns.RRHeader(b"example.com", ttl=60,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", ttl=60,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", ttl=120,
                         payload=dns.Record_A("1.2.3.4")),
        )

        # Vary the auth bit
        self._equalityTest(
            dns.RRHeader(b"example.com", auth=1,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", auth=1,
                         payload=dns.Record_A("1.2.3.4")),
            dns.RRHeader(b"example.com", auth=0,
                         payload=dns.Record_A("1.2.3.4")),
        )
    ...
    def test_naptr(self):
        ...
        self._equalityTest(
            dns.Record_NAPTR(1, 2, b"u", b"sip+E2U", b"/foo/bar/", b"baz", 12),
            dns.Record_NAPTR(1, 2, b"u", b"sip+E2U", b"/foo/bar/", b"baz", 12),
            dns.Record_NAPTR(1, 2, b"u", b"sip+E2U",
                             b"/bar/foo/", b"quux", 12),
        )
        # Vary the ttl
        self._equalityTest(
...
class NameToLabelsTests(unittest.SynchronousTestCase):
    ...
    def test_casePreservation(self):
        ...
        """
        self.assertEqual(dns._nameToLabels(b"EXAMPLE.COM"),
                         [b"EXAMPLE", b"COM", b""])
...
class OPTHeaderTests(ComparisonTestsMixin, unittest.TestCase):
    ...
    def test_nameReadonly(self):
        ...
        """
        h = dns._OPTHeader()
        self.assertRaises(AttributeError, setattr, h,
                          "name", dns.Name(b"example.com"))
    ...
    def test_udpPayloadSizeOverride(self):
        ...
        constructor.
        """
        self.assertEqual(dns._OPTHeader(
    ...
    def test_decodeDiscardsName(self):
        ...
        b = BytesIO(
            OPTNonStandardAttributes.bytes(
                excludeName=True) + b"\x07example\x03com\x00"
        )
...
class MessageNonAuthoritative:
    ...
    def kwargs(cls):
        ...
        return dict(
            ...
            ednsVersion=None,
            answers=[
                dns.RRHeader(b"", payload=dns.Record_A(
                    "1.2.3.4", ttl=0), auth=False)
            ],
        )
...
class EDNSMessageSpecificsTests(ConstructorTestsMixin, unittest.SynchronousTestCase):
    ...
    def test_reprNonDefautFields(self):
        ...
        """
        m = self.messageFactory(
            id=10, opCode=20, rCode=30, maxSize=40, ednsVersion=50)
        self.assertEqual(
            "<_EDNSMessage "
            "id=10 "
            "opCode=20 "
...
class StandardEncodingTestsMixin:
    ...
    def test_nonAuthoritativeMessageDecode(self):
        ...
        self.assertEqual(m, self.messageFactory(
...
class EDNSMessageEDNSEncodingTests(unittest.SynchronousTestCase):
    ...
    def test_extendedRcodeDecode(self):
        ...
        self.assertEqual(m, self.messageFactory(
            **MessageEDNSExtendedRCODE.kwargs()))
    ...
    def test_extendedRcodeZero(self):
        ...
        self.assertEqual(
            (15, 0),
            (standardMessage.rCode,
             standardMessage.additional[0].extendedRCODE),
        )
...
class CompactReprTests(unittest.SynchronousTestCase):
    ...
    def test_defaults(self):
        ...
        """
        self.assertEqual(
            "<Foo alwaysShowField='AS' flags=flagTrue>", repr(
                self.messageFactory())
        )
