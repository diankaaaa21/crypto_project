...
class CachingTests(unittest.TestCase):
    ...
    def test_constructorExpires(self):
        ...
        r = (
            [
                dns.RRHeader(
                    b"example.com", dns.A, dns.IN, 60, dns.Record_A(
                        "127.0.0.1", 60)
                )
            ],
            [
                dns.RRHeader(
                    b"example.com", dns.A, dns.IN, 50, dns.Record_A(
                        "127.0.0.1", 50)
                )
            ],
            [
                dns.RRHeader(
                    b"example.com", dns.A, dns.IN, 40, dns.Record_A(
                        "127.0.0.1", 40)
                )
            ],
        )
    ...
    def test_normalLookup(self):
        ...
        c.cacheResult(dns.Query(name=b"example.com",
                      type=dns.A, cls=dns.IN), r)
