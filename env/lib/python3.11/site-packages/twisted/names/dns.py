...
REV_TYPES = {v: k for (k, v) in chain(
    ...
    def encode(self, strio, compDict=None):
        ...
        RRHeader(
            ...
            ttl=(self.extendedRCODE << 24 |
                 self.version << 16 | self.dnssecOK << 15),
...
class _OPTVariableOption(tputil.FancyStrMixin, tputil.FancyEqMixin):
    ...
    def encode(self, strio, compDict=None):
        ...
        @param compDict: A dictionary of backreference addresses that
            ...
            be used for DNS name compression.
        """
        strio.write(struct.pack(self._fmt, self.code,
...
class Record_A6(tputil.FancyStrMixin, tputil.FancyEqMixin):
    ...
    def decode(self, strio, length=None):
        ...
        self.bytes = int((128 - self.prefixLen) / 8.0)
        if self.bytes:
            self.suffix = b"\x00" * (16 - self.bytes) + \
                readPrecisely(strio, self.bytes)
        if self.prefixLen:
...
class Record_SRV(tputil.FancyEqMixin, tputil.FancyStrMixin):
    ...
    def decode(self, strio, length=None):
        r = struct.unpack("!HHH", readPrecisely(
            strio, struct.calcsize("!HHH")))
...
class Record_TSIG(tputil.FancyEqMixin, tputil.FancyStrMixin):
    ...
    def encode(self, strio, compDict=None):
        ...
        strio.write(
            struct.pack("!HHH", self.originalID,
                        self.error, len(self.otherData))
        )
...
class Message(tputil.FancyEqMixin):
    ...
    def decode(self, strio, length=None):
        ...
        items = ((self.answers, nans), (self.authority, nns),
...
class DNSMixin:
    ...
    def _query(self, queries, timeout, id, writeMessage):
        ...
        cancelCall = self.callLater(
            timeout, self._clearFailed, resultDeferred, id)
