...
class CopyableFailure(failure.Failure, Copyable):
    ...
    def getStateToCopy(self):
        ...
        if isinstance(self.type, bytes):
            ...
            state["type"] = reflect.qual(self.type).encode(
                "utf-8")  # Exception class
...
class Broker(banana.Banana):
    ...
    def proto_version(self, vnum):
        ...
        if vnum != self.version:
            raise ProtocolError(
                f"Version Incompatibility: {self.version} {vnum}")
        ...
        for lobj in self.remotelyCachedObjects.values():
            ...
            try:
                cacheable.stoppedObserving(
                    perspective, RemoteCacheObserver(
                        self, cacheable, perspective)
                )
            except BaseException:
                log.deferr()
        ...
        if luid is None:
            if len(self.localObjects) > MAX_BROKER_REFS:
                ...
                if self.maxBrokerRefsViolations > 3:
                    ...
                    raise Error(
                        "Maximum PB reference count exceeded.  " "Goodbye.")
    ...
    def cacheRemotely(self, instance):
        ...
        self.remotelyCachedObjects[luid] = Local(
            instance, self.serializingPerspective)
        return luid
    ...
    def _sendMessage(self, prefix, perspective, objectID, message, args, kw):
        ...
        if "pbanswer" in kw:
            assert (not pbe) and (
                not pbc), "You can't specify a no-answer requirement."
            answerRequired = kw["pbanswer"]
            del kw["pbanswer"]
        if self.disconnected:
            raise DeadReferenceError("Calling Stale Broker")
        try:
            netArgs = self.serialize(
                args, perspective=perspective, method=message)
    ...
    def _recvMessage(
        ...
        try:
            ...
            netResult = object.remoteMessageReceived(
                self, message, netArgs, netKw)
        except Error as e:
            if answerRequired:
