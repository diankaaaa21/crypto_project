...
def createFactoryCopy(state):
    ...
    if stateId is None:
        raise RuntimeError(
            f"factory copy state has no 'id' member {repr(state)}")
    if stateId not in SimpleFactoryCopy.allIDs:
        raise RuntimeError(
            f"factory class has no ID: {SimpleFactoryCopy.allIDs}")
...
class NewStyleCachedTests(unittest.TestCase):
    def setUp(self):
        ...
        clientFactory = pb.PBClientFactory()
        reactor.connectTCP(
            "localhost", self.server.getHost().port, clientFactory)
...
class BrokerTests(unittest.TestCase):
    ...
    def test_copy(self):
        ...
        s.setNameForLocal("foo", foo)
        x = c.remoteForName("foo")
        x.callRemote("getCopy").addCallbacks(
            self.thunkResultGood, self.thunkErrorBad)
        pump.pump()
        pump.pump()
    ...
    def test_cache(self):
        ...
        coll = []
        o2.callRemote("getCache").addCallback(
            coll.append).addErrback(coll.append)
        o2.callRemote("getCache").addCallback(
            coll.append).addErrback(coll.append)
        complex = []
    ...
    def test_factoryCopy(self):
        ...
        x = c.remoteForName("foo")
        x.callRemote("getFactory", ID).addCallbacks(
            self.gotCopy, self.thunkResultBad)
        pump.pump()
        pump.pump()
        pump.pump()
        self.assertEqual(
...
class PagingTests(unittest.TestCase):
    ...
    def test_pagingWithCallback(self):
        ...
        s.setNameForLocal("foo", Pagerizer(
            finishedCallback, "hello", value=10))
        x = c.remoteForName("foo")
        l = []
    ...
    def test_filePagingWithCallback(self):
        ...
        """
        c, s, pump = connectedServerAndClient(test=self)
        pagerizer = FilePagerizer(
            self.filename, finishedCallback, "frodo", value=9)
        s.setNameForLocal("bar", pagerizer)
        x = c.remoteForName("bar")
        l = []
        util.getAllPages(x, "getPages").addCallback(l.append)
...
class NewCredTests(unittest.TestCase):
    ...
    def test_logoutAfterDecref(self):
        ...
        def cbLoggedOut(ignored):
            ...
            # without bound.
            self.assertEqual(
                self.serverFactory.protocolInstance._localCleanup, {})
...
class NSPTests(unittest.TestCase):
    ...
    def test_NSP(self):
        ...
        d = factory.login(credentials.UsernamePassword(
            b"user", b"pass"), "BRAINS!")
