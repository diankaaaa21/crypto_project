...
from os.path import abspath, basename, dirname, exists, isabs, islink
...
class FilePath(AbstractFilePath[AnyStr]):
    ...
    if TYPE_CHECKING:
        ...
        def sibling(
            self: _Self, path: OtherAnyStr) -> FilePath[OtherAnyStr]: ...
    ...
    def child(self, path: OtherAnyStr) -> FilePath[OtherAnyStr]:
        ...
        if sep in norm:
            raise InsecurePath(
                f"{path!r} contains one or more directory separators")
    ...
    def globChildren(self, pattern: OtherAnyStr) -> List[FilePath[OtherAnyStr]]:
        ...
        path = ourPath[-1] == sep and ourPath + \
            pattern or sep.join([ourPath, pattern])
    ...
    def temporarySibling(
        ...
        sib = self.sibling(
            ...
            self.clonePath(ourPath).basename() + ext
        )
        sib.requireCreate()
        return sib
    ...
    def moveTo(
        ...
        """
        try:
            os.rename(self._getPathAsSameTypeAs(
