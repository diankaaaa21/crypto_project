...
class CoerceParameter:
    ...
    def dispatch(self, parameterName, value):
        ...
        if value is None:
            raise UsageError(
                f"Parameter '{parameterName}' requires an argument.")
        try:
...
class Options(Dict[str, Any]):
    ...
    def parseOptions(self, options=None):
        ...
        if len(options) > 1 and options[-2] == "--_shell-completion":
            ...
            _shellcomp.shellComplete(
                self, cmdName, options, self._shellCompFile)
            sys.exit(0)
    ...
    def _generic_flag(self, flagName, value=None):
        if value not in ("", None):
            raise UsageError(
                "Flag '%s' takes no argument." ' Not even "%s".' % (
                    flagName, value)
            )
    ...
    def _gather_parameters(self):
        ...
        reflect.accumulateClassList(
            self.__class__, "optParameters", parameters)
    ...
    def _gather_handlers(self):
        ...
        for name in dct.keys():
            ...
            # same signature.
            if takesArg:
                def fn(name, value, m=method): return m(value)
            else:
                # XXX: This won't raise a TypeError if it's called
                # with a value when it shouldn't be.
                def fn(name, value=None, m=method): return m()
