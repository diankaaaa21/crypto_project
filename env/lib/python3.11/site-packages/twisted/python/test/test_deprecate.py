...
from twisted.python.deprecate import DEPRECATION_WARNING_FORMAT, _appendToDocstring
...
"""
    ...
    def test_warningLineNumberDisFindlinestarts(self):
        ...
        self.assertSamePath(
            FilePath(warningsShown[0]["filename"].encode("utf-8")),
            self.package.sibling(
                b"twisted_private_helper").child(b"pep626.py"),
        )
...
class DeprecationWarningsTests(SynchronousTestCase):
    ...
    def test_deprecateEmitsWarning(self):
        ...
        with catch_warnings(record=True) as caught:
            ...
            self.assertEqual(caught[0].filename.rstrip(
                "co"), __file__.rstrip("co"))
    ...
    def test_getDeprecationDocstring(self):
        ...
        version = Version("Twisted", 8, 0, 0)
        self.assertEqual(
            "Deprecated in Twisted 8.0.0.", _getDeprecationDocstring(
                version, "")
        )
...
class DeprecatedDecoratorTests(SynchronousTestCase):
    ...
    def assertDocstring(self, target, expected):
        ...
        self.assertEqual(
            expected, [x.strip()
                       for x in target.__doc__.splitlines() if x.strip()]
        )
...
class MutualArgumentExclusionTests(SynchronousTestCase):
    ...
    def test_passed_starStar(self):
        ...
        self.assertEqual(
            self.checkPassed(func, 1, x=2, y=3, z=4), dict(
                a=1, b=dict(x=2, y=3, z=4))
        )
    ...
    def test_passed_noDefaultValues(self):
        ...
        self.assertEqual(self.checkPassed(
            func, 1, 2, e=7), dict(a=1, b=2, e=7))
...
class KeywordOnlyTests(SynchronousTestCase):
    ...
    def test_passedKeywordOnly(self):
        ...
        self.assertEqual(self.checkPassed(func1, 1, 2, 3),
                         dict(a=(1, 2, 3), b=True))
        self.assertEqual(
            self.checkPassed(func1, 1, 2, 3, b=False), dict(
                a=(1, 2, 3), b=False)
        )
        self.assertEqual(
