...
class FixedOffsetTimeZoneTests(TestCase):
    ...
    def test_tzinfo(self) -> None:
        ...
        def testForTimeZone(
            ...
            self.assertEqual(tzDST.tzname(localDSTdt),
                             f"UTC{expectedOffsetDST}")
            self.assertEqual(tzSTD.tzname(localSTDdt),
                             f"UTC{expectedOffsetSTD}")
            ...
            self.assertEqual(
                tzDST.utcoffset(localDSTdt), timeDeltaFromOffset(
                    expectedOffsetDST)
            )
            self.assertEqual(
                tzSTD.utcoffset(localSTDdt), timeDeltaFromOffset(
                    expectedOffsetSTD)
            )
