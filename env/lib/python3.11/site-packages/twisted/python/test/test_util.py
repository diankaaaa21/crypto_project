...
class NameToLabelTests(TestCase):
    ...
    def test_nameToLabel(self):
        ...
        for inp, out in nameData:
            ...
            self.assertEqual(
                got, out, f"nameToLabel({inp!r}) == {got!r} != {out!r}")
...
class SwitchUIDTests(TestCase):
    ...
    def test_currentUID(self):
        ...
        self.assertIn("but uid is already %i" %
                      uid, currentWarnings[0]["message"])
    ...
    def test_currentEUID(self):
        ...
        self.assertIn("but euid is already %i" %
                      euid, currentWarnings[0]["message"])
...
class InsensitiveDictTests(TestCase):
    ...
    def test_preserve(self):
        ...
        """
        dct = util.InsensitiveDict(
            {"Foo": "bar", 1: 2, "fnz": {1: 2}}, preserve=1)
    ...
    def test_noPreserve(self):
        ...
        """
        dct = util.InsensitiveDict(
            {"Foo": "bar", 1: 2, "fnz": {1: 2}}, preserve=0)
        keys = ["foo", "fnz", 1]
        for x in keys:
            self.assertIn(x, dct.keys())
            self.assertIn((x, dct[x]), dct.items())
...
class RunAsEffectiveUserTests(TestCase):
    ...
    def _securedFunction(self, startUID, startGID, wantUID, wantGID):
        ...
        """
        self.assertTrue(wantUID == startUID or wantUID ==
                        self.mockos.seteuidCalls[-1])
        self.assertTrue(wantGID == startGID or wantGID ==
                        self.mockos.setegidCalls[-1])
    ...
    def test_takesKeyworkArguments(self):
        ...
        function.
        """
        result = util.runAsEffectiveUser(
            0, 0, lambda x, y=1, z=1: x * y * z, 2, z=3)
        self.assertEqual(result, 6)
...
class SuppressedWarningsTests(TestCase):
    ...
    def test_runWithWarningsSuppressedUnfiltered(self):
        ...
        self.assertEqual(["don't ignore"], [w["message"]
    ...
    def test_noSideEffects(self):
        ...
        self.assertEqual(["ignore foo"], [w["message"]
                         for w in self.flushWarnings()])
...
class FancyStrMixinTests(TestCase):
    ...
    def test_formatter(self):
        ...
        class Foo(util.FancyStrMixin):
            showAttributes = (
                "first", ("second", lambda value: repr(value[::-1])))
            first = "hello"
            second = "world"
