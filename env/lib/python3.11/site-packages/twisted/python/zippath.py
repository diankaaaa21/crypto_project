...
class ZipPath(Generic[_ZipStr, _ArchiveStr], AbstractFilePath[_ZipStr]):
    ...
    def parent(self) -> Union[ZipPath[_ZipStr, _ArchiveStr], ZipArchive[_ZipStr]]:
        ...
        if isinstance(parent, ZipArchive):
            return ZipArchive(
                _coerceToFilesystemEncoding(
                    self.path, self.archive._zipfileFilename)
            )
        return parent
    ...
    def sibling(self, path: OtherAnyStr) -> ZipPath[OtherAnyStr, _ArchiveStr]:
        ...
        rightTypedParent: Union[ZipPath[_ZipStr,
                                        _ArchiveStr], ZipArchive[_ArchiveStr]]

        parent = self.parent()
        rightTypedParent = self.archive if isinstance(
            parent, ZipArchive) else parent
...
class ZipArchive(ZipPath[AnyStr, AnyStr]):
    ...
    def __init__(self, archivePathname: AnyStr) -> None:
        ...
        self.zipfile = ZipFile(
            _coerceToFilesystemEncoding("", archivePathname))
        zfname = self.zipfile.filename
        assert (
            zfname is not None
        ), "zipfile must have filename when initialized with a path"
        self._zipfileFilename = _coerceToFilesystemEncoding(
            archivePathname, zfname)
        ...
        for name in self.zipfile.namelist():
            splitName = _coerceToFilesystemEncoding(
                self.path, name).split(self.sep)
