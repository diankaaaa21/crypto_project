...
class AbstractFilePathTests(BytesTestCase):
    ...
    def test_segmentsFromPositive(self) -> None:
        ...
        """
        self.assertEqual(
            self.path.child(b"a").child(b"b").child(
                b"c").segmentsFrom(self.path),
            [b"a", b"b", b"c"],
        )
    ...
    def test_dictionaryKeyWithString(self) -> None:
        ...
        """
        f1 = self.path.child(b"file1")
        dictoid: Dict[Union[filepath.FilePath[bytes], bytes], str] = {
            f1: "hello"}
...
class PermissionsTests(BytesTestCase):
            ...
            for w in (True, False):
                for x in (True, False):
                    self.assertEqual(filepath.RWX(r, w, x),
                                     filepath.RWX(r, w, x))
                    self.assertNotUnequal(filepath.RWX(
                        r, w, x), filepath.RWX(r, w, x))
        self.assertNotEqual(
    ...
    def test_permissionsEq(self) -> None:
        ...
        self.assertNotUnequal(filepath.Permissions(
            0o777), filepath.Permissions(0o777))
...
class FilePathTests(AbstractFilePathTests):
    ...
    def test_chmod(self) -> None:
        ...
        for mode in (0o555, 0o777):
            ...
            self.assertEqual(
                stat.S_IMODE(
                    os.stat(self.path.child(b"sub1").path).st_mode), mode
            )
    ...
    def createLinks(self) -> None:
        ...
        os.symlink(
            subdir.child(b"file2").path, self._mkpath(
                b"sub1", b"sub1.file2.link")
        )
    ...
    def test_realpathSymlink(self) -> None:
        ...
        self.createLinks()
        os.symlink(
            self.path.child(b"file2.link").path, self.path.child(
                b"link.link").path
        )
        self.assertEqual(
    ...
    def test_getPermissions_POSIX(self) -> None:
        ...
        for mode in (0o777, 0o700):
            self.path.child(b"sub1").chmod(mode)
            self.assertEqual(
                self.path.child(b"sub1").getPermissions(
...
class UnicodeFilePathTests(TestCase):
    ...
    def test_selfSiblingExtensionSearch(self) -> None:
        ...
        self.assertEqual(exists.siblingExtensionSearch(b""),
                         exists.asBytesMode())
        self.assertEqual(exists.siblingExtensionSearch(""),
                         exists.asTextMode())
