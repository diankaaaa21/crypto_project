...
class WrapperTests(unittest.TestCase):
    ...
    def test_transportInterfaces(self):
        ...
        wrapper = policies.ProtocolWrapper(
            policies.WrappingFactory(None), proto)
...
class ThrottlingTests(unittest.TestCase):
    ...
    def test_writeLimit(self):
        ...
        """
        server = Server()
        tServer = TestableThrottlingFactory(
            task.Clock(), server, writeLimit=10)
        port = tServer.buildProtocol(
            address.IPv4Address("TCP", "127.0.0.1", 0))
...
class LoggingFactoryTests(unittest.TestCase):
    ...
    def test_thingsGetLogged(self):
        ...
        v = f.openFile.getvalue()
        self.assertIn("SV 1: {!r}".format(
            [b"prepare for vector! to the extreme"]), v)
