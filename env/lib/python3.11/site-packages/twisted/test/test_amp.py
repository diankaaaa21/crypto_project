...
class DeferredSymmetricCommandProtocol(SimpleSymmetricCommandProtocol):
    def switchit(self, name):
        if name == b"test-proto":
            self.maybeLaterProto = TestProto(
                self.onConnLost, SWITCH_SERVER_DATA)
...
class CommandDispatchTests(TestCase):
    ...
    def test_receivedAsk(self):
        ...
        input = amp.Box(_command="hello",
                        _ask="test-command-id", hello="world")
...
class CommandLocatorTests(TestCase):
    ...
    def _checkSimpleGreeting(self, locatorClass, expected):
        ...
        def done(values):
            self.assertEqual(values, amp.AmpBox(
                cookieplus=b"%d" % (expected,)))
    ...
    def test_lookupFunctionDeprecatedOverride(self):
        ...
        result = normalResponderObject(
            amp.Box(greeting=b"ni hao", cookie=b"5"))
...
class AMPTests(TestCase):
    ...
    def test_unknownCommandLow(self):
        ...
        c.callRemoteString(b"WTF").addErrback(
            clearAndAdd).addCallback(L.append)
        p.flush()
    ...
    def test_keyTooLong(self):
        ...
        c, s, p = connectedServerAndClient()
        x = "H" * (0xFF + 1)
        tl = self.assertRaises(
            amp.TooLong, c.callRemoteString, b"Hello", **{x: b"hi"})
...
class CommandInheritanceTests(TestCase):
    ...
    def errorCheck(self, err, proto, cmd, **kw):
        ...
        """
        c, s, p = connectedServerAndClient(
            ServerClass=proto, ClientClass=proto)
...
class LiveFireBase:
    ...
    def setUp(self):
        ...
        dl = defer.DeferredList(
            [self.clientFactory.onMade, self.serverFactory.onMade])
...
class LiveFireTLSTests(LiveFireBase, TestCase):
    ...
    def test_liveFireCustomTLS(self):
        ...
        return self.cli.callRemote(
            amp.StartTLS, tls_localCertificate=cert, tls_verifyAuthorities=[
                cert]
        ).addCallback(secured)
    ...
    def test_responderCallsParseArguments(self):
        ...
        response.addCallback(
            lambda ign: self.assertEqual(
                protocol.parseArgumentsArguments, ({
...
class ListOfTestsMixin:
    ...
    def test_toBox(self):
        ...
        for key in self.objects:
            stringList.toBox(key.encode("ascii"), strings,
                             self.objects.copy(), None)
...
class RemoteAmpErrorTests(TestCase):
    ...
    def test_stringMessageReplacesNonAsciiText(self):
        ...
        self.assertEqual(
            "Code<BROKEN-\\xff>: Something has broken", str(error))
