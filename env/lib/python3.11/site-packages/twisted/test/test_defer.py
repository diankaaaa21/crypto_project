...
class DeferredTests(unittest.SynchronousTestCase, ImmediateFailureMixin):
    ...
    def test_addCallbacksNoneErrback(self) -> None:
        ...
        deferred.addCallbacks(self._callback, cast(
    ...
    def test_cancelDeferredListWithFireOnOneErrback(self) -> None:
        ...
        self.failureResultOf(deferredOne, defer.CancelledError)
        self.failureResultOf(deferredTwo, defer.CancelledError)
        deferredListFailure = self.failureResultOf(
            deferredList, defer.FirstError)
    ...
    def testCallbackErrors(self) -> None:
        ...
        l = []
        d2 = Deferred[int]()
        d2.addCallback(lambda _: Failure(
            ZeroDivisionError())).addErrback(l.append)
        d2.callback(1)
        self.assertIsInstance(l[0].value, ZeroDivisionError)
    ...
    def test_callbackReturnsDeferred(self) -> None:
        ...
        d2.unpause()
        self.assertIsNotNone(self.callbackResults,
    ...
    def test_maybeDeferredSyncException(self) -> None:
        ...
        results: List[int] = []
        errors: List[Failure] = []
        defer.maybeDeferred(raisesException).addCallbacks(
    ...
    def test_reprWithResult(self) -> None:
        ...
        d.callback("orange")
        self.assertEqual(
            repr(d), f"<Deferred at 0x{id(d):x} current result: 'orange'>")
    ...
    def test_fromCoroutineRequiresCoroutine(self) -> None:
        ...
        for thing in thingsThatAreNotCoroutines:
            self.assertRaises(defer.NotACoroutineError,
                              Deferred.fromCoroutine, thing)
    ...
    def test_DEFERRED_SUBCLASSES(self) -> None:
        ...
        """
        self.assertEqual(defer._DEFERRED_SUBCLASSES[:2], [
...
class CoroutineContextVarsTests(unittest.TestCase):
    ...
    async def test_asyncWithLock(self) -> None:
        ...
        """
        lock = DeferredLock()
        # This makes mypy check that async with doesn't suppress exceptions.
        d = None
        async with lock:
            self.assertTrue(lock.locked)
            d = lock.acquire()
