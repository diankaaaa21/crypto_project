    ...
    def processEnded(self, reason):
        ...
        if v.exitCode is not None:
            return self.deferred.errback(
                ValueError(
                    f"SIG{self.signal}: exitCode is {v.exitCode}, not None")
            )
...
class FDChecker(protocol.ProcessProtocol):
    ...
    def childDataReceived(self, childFD, data):
        ...
        if self.state == 3:
            if childFD != 1:
                self.fail(
                    f"read '{childFD}' on fd {data} (not 1) during state 3")
                return
            self.data += data
            if len(self.data) == 6:
    ...
    def childConnectionLost(self, childFD):
        ...
        if self.state == 2:
            if childFD != 4:
                self.fail(
                    "got connectionLost(%d) (not 4) during state 2" % childFD)
                return
            self.state = 3
...
class PosixProcessBase:
    ...
    def test_executionError(self):
        ...
        try:
            reactor.spawnProcess(
                p, cmd, [b"false"], env=None, usePTY=self.usePTY)
...
class MockProcessTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patch(process.Process, "_trySpawnInsteadOfFork",
                   lambda *a, **k: False)
    ...
    def test_mockForkErrorClosePTY(self):
        ...
        self.mockos.raiseFork = OSError(errno.EAGAIN, None)
        protocol = TrivialProcessProtocol(None)
        self.assertRaises(OSError, reactor.spawnProcess,
    ...
    def test_mockSetUid(self):
        ...
        try:
            reactor.spawnProcess(
                p, cmd, [b"ouch"], env=None, usePTY=False, uid=8080)
        except SystemError:
            self.assertTrue(self.mockos.exited)
            self.assertEqual(
                self.mockos.actions,
    ...
    def test_mockPTYSetUid(self):
        ...
        try:
            reactor.spawnProcess(
                p, cmd, [b"ouch"], env=None, usePTY=True, uid=8081)
        except SystemError:
            self.assertTrue(self.mockos.exited)
            self.assertEqual(
                self.mockos.actions,
    ...
    def test_mockPTYSetUidInParent(self):
        ...
        try:
            ...
            reactor.spawnProcess(
                p, cmd, [b"ouch"], env=None, usePTY=True, uid=8080)
        finally:
    ...
    def test_stderr(self):
        ...
        reactor.spawnProcess(
            ...
            [
                ...
                b"-c",
                networkString(
                    "import sys; sys.stderr.write" "('{}')".format(value)),
            ],
            env=None,
            path="/tmp",
            usePTY=self.usePTY,
...
class Win32ProcessTests(unittest.TestCase):
    ...
    def test_badArgs(self):
        pyArgs = [pyExe, b"-u", b"-c", b"print('hello')"]
        p = Accumulator()
        self.assertRaises(ValueError, reactor.spawnProcess,
                          p, pyExe, pyArgs, uid=1)
        self.assertRaises(ValueError, reactor.spawnProcess,
                          p, pyExe, pyArgs, gid=1)
        self.assertRaises(ValueError, reactor.spawnProcess,
                          p, pyExe, pyArgs, usePTY=1)
        self.assertRaises(
            ValueError, reactor.spawnProcess, p, pyExe, pyArgs, childFDs={
                1: "r"}
        )
...
class Win32UnicodeEnvironmentTests(unittest.TestCase):
    ...
    def test_UTF8StringInEnvironment(self):
        ...
        def gotEnvironment(environb):
            ...
            self.assertEqual(
                environb[envKeyBytes], "Speciał Committór".encode(
                    sys.stdout.encoding)
            )
...
class Win32CreateProcessFlagsTests(unittest.TestCase):
    ...
    def test_flags(self):
        ...
        self.patch(_dumbwin32proc.win32process,
                   "CreateProcess", fakeCreateprocess)
        exe = sys.executable
...
class ClosingPipesTests(unittest.TestCase):
    ...
    def _endProcess(self, reason, p):
        ...
        self.assertNotEqual(reason.exitCode, 42,
