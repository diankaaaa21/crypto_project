...
class WSGITestsMixin:
    ...
    def lowLevelRender(
        ...
        root = WSGIResource(self.reactor, self.threadpool,
                            applicationFactory())
        ...
        if query is not None:
            uri += "?" + "&".join(
                ["=".join([urlquote(k, safe), urlquote(v, safe)])
                 for (k, v) in query]
            )
        request.requestReceived(
            _toByteString(method), _toByteString(
                uri), b"HTTP/" + _toByteString(version)
        )
        return request
    ...
    def render(self, *a, **kw):
        ...
        self.lowLevelRender(Request, applicationFactory,
                            channelFactory, *a, **kw)
        return result
...
class EnvironTests(WSGITestsMixin, TestCase):
    ...
    def test_scriptName(self):
        ...
        unencoded.addCallback(self.environKeyEqual(
            "SCRIPT_NAME", "/foo///bar\xff"))
    ...
    def test_pathInfoIsNativeString(self):
        ...
        # t.w.wsgi has any say in the matter.
        request, result = self.prepareRequest()
        self.assertRaises(TypeError, request.requestReceived,
                          path="/res/foo/bar")
    ...
    def test_contentTypeIsNativeString(self):
        ...
        for contentType in b"x-foo/bar", "x-foo/bar":
            ...
            request.requestReceived()
            result.addCallback(self.environKeyEqual(
    ...
    def test_serverName(self):
        ...
        present = self.render("GET", "1.1", [], [""], None, [
                              ("host", "example.org")])
    ...
    def test_wsgiVersion(self):
        ...
        versionDeferred.addCallback(
            self.environKeyEqual("wsgi.version", (1, 0)))
    ...
    def test_wsgiURLScheme(self):
        ...
        httpDeferred.addCallback(
            self.environKeyEqual("wsgi.url_scheme", "http"))

        self.channelFactory = channelFactory
        httpsDeferred = self.render("GET", "1.1", [], [""])
        httpsDeferred.addCallback(
            self.environKeyEqual("wsgi.url_scheme", "https"))
...
class InputStreamTestMixin(WSGITestsMixin):
    ...
    def test_readSome(self):
        ...
        bytes = b"hello, world."
        d = self._renderAndReturnReaderResult(
            lambda input: input.read(3), bytes)
...
class StartResponseTests(WSGITestsMixin, TestCase):
    ...
    def test_content(self):
        ...
        def cbRendered(ignored):
            self.assertEqual(self.getContentFromResponse(
                intermediateValues[0]), b"foo")
            self.assertEqual(
    ...
    def test_startResponseWithException(self):
        ...
        def applicationFactory():
            def application(environ, startResponse):
                startResponse(
                    "100 Foo", [], (Exception, Exception("foo"), None))
                return iter(())
    ...
    def test_startResponseWithExceptionTooLate(self):
        ...
        def cbRendered(ignored):
            ...
            tb2 = excInfo[2]
            self.assertEqual(traceback.extract_tb(
                tb1)[1], traceback.extract_tb(tb2)[0])
