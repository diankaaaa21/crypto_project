...
class XMLRPCTests(unittest.TestCase):
    def setUp(self):
        self.p = reactor.listenTCP(
            0, server.Site(Test()), interface="127.0.0.1")
    ...
    def test_errors(self):
        ...
        filtered = FilteringLogObserver(
            logObserver, [LogLevelFilterPredicate(
        ...
        for code, methodName in [
            ...
            d.addCallback(lambda exc, code=code: self.assertEqual(
                exc.faultCode, code))
            dl.append(d)
    ...
    def test_errorGet(self):
        ...
        d = agent.request(b"GET", networkString(
    ...
    def test_doubleEncodingError(self):
        ...
        def cbFailed(ignored):
            ...
            self.assertIsInstance(
                logObserver[0]["log_failure"].value, RuntimeError)
...
class XMLRPCPublicLookupProcedureTests(unittest.TestCase):
    ...
    def createServer(self, resource):
        ...
        self.port = self.p.getHost().port
        self.proxy = xmlrpc.Proxy(networkString(
            "http://127.0.0.1:%d" % self.port))
...
class XMLRPCIntrospectionTests(XMLRPCTests):
    def setUp(self):
        ...
        self.p = reactor.listenTCP(
            0, server.Site(xmlrpc), interface="127.0.0.1")
        self.port = self.p.getHost().port
        self.factories = []
