...
class StaticFileTests(TestCase):
    ...
    def test_undecodablePath(self):
        ...
        def cbRendered(ignored):
            self.assertEqual(request.responseCode, 404)
            self.assertEqual(
                len(self.flushLoggedErrors(UnicodeDecodeError)), 1)
    ...
    def test_forbiddenResource_default(self):
        ...
        """
        self.assertIsInstance(static.File(b".").forbidden,
...
class StaticMakeProducerTests(TestCase):
    ...
    def test_mutipleRangeSetsContentHeaders(self):
        ...
        with resource.openForReading() as file:
            ...
            self.assertEqual(
                {b"content-length",
                    b"content-type"}, set(contentHeaders.keys())
            )
            ...
            self.assertNotIdentical(
                None,
                re.match(
                    rb'multipart/byteranges; boundary="[^"]*"\Z', contentType),
            )
...
class NoRangeStaticProducerTests(TestCase):
    ...
    def test_implementsIPullProducer(self):
        ...
        """
        verifyObject(interfaces.IPullProducer,
...
class RangeTests(TestCase):
    ...
    def _assertLogged(self, expected):
        ...
        self.assertEqual(self.catcher, [],
                         f"An additional log occurred: {logItem!r}")
    ...
    def test_rangeMissingStop(self):
        ...
        """
        self.assertEqual(self.resource._parseRangeHeader(
            b"bytes=0-"), [(0, None)])
    ...
    def test_multipleRangeRequest(self):
        ...
        self.request.requestHeaders.addRawHeader(
            b"range", b"bytes=" + rangeHeaderValue)
        self.resource.render(self.request)
        ...
        parts = self.parseMultipartBody(
            b"".join(self.request.written), boundary)
        self.assertEqual(len(startEnds), len(parts))
        for part, (s, e) in zip(parts, startEnds):
            self.assertEqual(networkString(
                self.resource.type), part[b"contentType"])
    ...
    def test_implicitEnd(self):
        ...
        self.assertEqual(
            self.request.responseHeaders.getRawHeaders(
                b"content-length")[0], b"41"
        )
...
class DirectoryListerTests(TestCase):
    ...
    def test_escapeHeader(self):
        ...
        self.assertIn(b"<h1>Directory listing for foo&amp;bar</h1>", data)
        self.assertIn(
            b"<title>Directory listing for foo&amp;bar</title>", data)
