...
def _getMultiPartArgs(content: bytes, ctype: bytes) -> dict[bytes, list[bytes]]:
    ...
    for part in msg.get_payload():  # type:ignore[assignment]
        ...
        if not name:
            continue
        payload: bytes = part.get_payload(
            decode=True)  # type:ignore[assignment]
...
class Request:
        ...
        if self.producer:
            self._log.failure(
                ...
                Failure(RuntimeError(
                    f"Producer was not unregistered for {self.uri}")),
            )
    ...
    def finish(self):
        ...
        if self.finished:
            warnings.warn(
                "Warning! request.finish called twice.", stacklevel=2)
            return
    ...
    def write(self, data):
        ...
        if not self.startedWriting:
            ...
            self.channel.writeHeaders(
                version, code, reason, self.responseHeaders)
    ...
    def _dataReceived_TRAILER(self) -> bool:
        ...
        if eolIndex == -1:
            ...
            if minTrailerSize > self._maxTrailerHeadersSize:
                raise _MalformedChunkedDataError(
                    "Trailer headers data is too long.")
...
class HTTPChannel(basic.LineReceiver, policies.TimeoutMixin):
    ...
    def timeoutConnection(self):
        self._log.info("Timing out client: {peer}", peer=str(
...
def combinedLogFormatter(timestamp, request):
    ...
    if isinstance(
        clientAddr, (address.IPv4Address, address.IPv6Address,
...
class _XForwardedForRequest(proxyForInterface(IRequest, "_request")):
    ...
    def getClientAddress(self):
        ...
        """
        host = (
            self._request.requestHeaders.getRawHeaders(
                b"X-Forwarded-For", [b"-"])[0]
            .split(b",")[0]
            .strip()
        )
...
class HTTPFactory(protocol.ServerFactory):
    ...
    def startFactory(self) -> None:
        ...
        self._logFile = self._openLogFile(
            self.logPath) if self.logPath else log.logfile
