...
class H2Connection(Protocol, TimeoutMixin):
    ...
    def _requestAborted(self, event):
        ...
        stream.connectionLost(
            Failure(ConnectionLost("Stream reset with code %s" %
                    event.error_code))
        )
    ...
    def _respondToBadRequestAndDisconnect(self, streamID):
        ...
        self.conn.send_headers(
            headers=headers, stream_id=streamID, end_stream=True)
        stillActive = self._tryToWriteControlData()
        if stillActive:
...
class H2Stream:
    ...
    def _convertHeaders(self, headers):
        ...
        for header in headers:
            if not header[0].startswith(b":"):
                gotLength = _addHeaderToRequest(
                    self._request, header) or gotLength
    ...
    def writeHeaders(self, version, code, reason, headers):
        ...
        self._conn.writeHeaders(
            ...
            code,
            reason,
            [(k, v) for (k, values) in headers.getAllRawHeaders()
             for v in values],
            self.streamID,
        )
