...
class _AgentBase:
    ...
    def _requestWithEndpoint(
        ...
        if not isinstance(method, bytes):
            raise TypeError(
                f"method={method!r} is {type(method)}, but must be bytes")
...
class _FakeStdlibRequest(_RequestBase):
    ...
    def add_unredirected_header(self, name, value):
        self._twistedHeaders.addRawHeader(
            networkString(name), networkString(value))

    def get_full_url(self):
        return self.uri

    def get_header(self, name, default=None):
        headers = self._twistedHeaders.getRawHeaders(
    ...
    def request(
        ...
        if not actualHeaders.hasHeader(b"Cookie"):
            ...
            if cookieHeader is not None:
                ...
                actualHeaders.addRawHeader(
                    b"Cookie", networkString(cookieHeader))
...
class RedirectAgent:
    ...
    def __init__(
        ...
        sensitive = {_canonicalHeaderName(each)
                     for each in sensitiveHeaderNames}
