...
class Resource:
    ...
    def putChild(self, path: bytes, child: IResource) -> None:
        ...
        if not isinstance(path, bytes):
            raise TypeError(
                f"Path segment must be bytes, but {path!r} is {type(path)}")
...
class _UnsafeNoResource(_UnsafeErrorPageBase):
    ...
    def __init__(self, message="Sorry. No luck finding that resource."):
        _UnsafeErrorPageBase.__init__(
            self, NOT_FOUND, "No Such Resource", message)
...
class _UnsafeForbiddenResource(_UnsafeErrorPageBase):
    ...
    def __init__(self, message="Sorry, resource is forbidden."):
        _UnsafeErrorPageBase.__init__(
            self, FORBIDDEN, "Forbidden Resource", message)
