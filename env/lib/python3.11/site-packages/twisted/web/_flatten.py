...
def _flattenElement(
    ...
    if isinstance(root, (bytes, str)):
        ...
        for k, v in root.attributes.items():
            ...
            yield keepGoing(
                v, attributeEscapingDoneOutside, write=writeWithAttributeEscaping(
                    write)
            )
            write(b'"')
        ...
        yield keepGoingAsync(
            Deferred.fromCoroutine(
                cast(Coroutine[Deferred[Flattenable],
                     object, Flattenable], root)
            )
        )
...
async def _flattenTree(
    ...
    stack: List[Generator[Any, Any, Any]] = [
        _flattenElement(request, root, bufferedWrite,
                        [], None, escapeForContent)
    ]
