...
_ignoreDecoderErrors = _moduleLog.failureHandler(
        ...
        if self.response.code in self.NO_BODY_CODES or self.request.method == b"HEAD":
            ...
            if transferEncodingHeaders:
                ...
                if contentLength == 0:
                    ...
                    def transferDecoder(x, y): return _IdentityTransferDecoder(
                        contentLength, x, y
                    )
            ...
            if transferDecoder is None:
                ...
                self.switchToBodyMode(
                    transferDecoder(
                        self.response._bodyDataReceived, self._finished)
                )
...
def makeStatefulDispatcher(name, template):
    ...
    def dispatcher(self, *args, **kwargs):
        ...
        if func is None:
            raise RuntimeError(
                f"{self!r} has no {name} method in state {self._state}")
...
class Response:
    ...
    _bodyDataReceived = makeStatefulDispatcher(
        "bodyDataReceived", _bodyDataReceived)
    ...
    _bodyDataFinished = makeStatefulDispatcher(
        "bodyDataFinished", _bodyDataFinished)
