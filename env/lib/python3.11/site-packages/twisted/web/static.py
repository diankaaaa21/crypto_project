...
class File(resource.Resource, filepath.FilePath[str]):
    ...
    def getChild(self, path, request):
        ...
        if isinstance(path, bytes):
            try:
                ...
                log.err(
                    None, f"Could not decode path segment as utf-8: {path!r}")
    ...
    def _parseRangeHeader(self, range):
        ...
        unparsedRanges = list(
            filter(None, map(bytes.strip, value.split(b","))))
        parsedRanges = []
    ...
    def _doSingleRangeRequest(self, request, startAndEnd):
        ...
        if offset == size == 0:
            ...
            request.setHeader(
                b"content-range", networkString("bytes */%d" %
                                                (self.getFileSize(),))
            )
        else:
    ...
    def _doMultipleRangeRequest(self, request, byteRanges):
        ...
        boundary = networkString(
            f"{int(time.time() * 1000000):x}{os.getpid():x}")
        if self.type:
        ...
        request.setHeader(
            b"content-type",
            networkString(
                f'multipart/byteranges; boundary="{nativeString(boundary)}"'),
        )
        request.setHeader(
    ...
    def createSimilarFile(self, path):
        f = self.__class__(path, self.defaultType,
                           self.ignoredExts, self.registry)
