...
class Headers:
    ...
    def __cmp__(self, other):
        ...
        if isinstance(other, Headers):
            return cmp(
                sorted(self._rawHeaders.items()), sorted(
                    other._rawHeaders.items())
            )
...
class _NameEncoder:
    ...
    def encode(self, name: Union[str, bytes]) -> bytes:
        ...
        bytes_name = name.encode(
            "iso-8859-1") if isinstance(name, str) else name

        if not _istoken(bytes_name):
            raise InvalidHeaderName(bytes_name)

        result = b"-".join([word.capitalize()
                           for word in bytes_name.split(b"-")])
