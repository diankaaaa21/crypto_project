...
class FilteringLogObserverTests(unittest.TestCase):
    ...
    def test_shouldLogEventYesYesNoFilter(self) -> None:
        ...
        results.
        """
        self.assertEqual(self.filterWith(
            ["twoPlus", "twoMinus", "no"]), [0, 1, 2, 3])
...
class LogLevelFilterPredicateTests(unittest.TestCase):
        ...
        for default in ("", cast(str, None)):
            self.assertEqual(
                predicate.logLevelForNamespace(
                    default), predicate.defaultLogLevel
            )
            self.assertEqual(
    ...
    def test_setLogLevel(self) -> None:
        ...
        for default in ("", cast(str, None)):
            ...
            self.assertEqual(
                predicate.logLevelForNamespace(""), LogLevel.error)
            self.assertEqual(
                predicate.logLevelForNamespace(cast(str, None)), LogLevel.error
            )
            self.assertEqual(predicate.logLevelForNamespace(
                "twisted"), LogLevel.error)
            self.assertEqual(
            ...
            self.assertEqual(
                predicate.logLevelForNamespace(
                    "twext.web2.dav.test"), LogLevel.warn
            )
            self.assertEqual(
    ...
    def test_clearLogLevels(self) -> None:
        ...
        self.assertEqual(
            predicate.logLevelForNamespace(
                "twisted"), predicate.defaultLogLevel
        )
        self.assertEqual(
            predicate.logLevelForNamespace(
                "twext.web2"), predicate.defaultLogLevel
        )
        self.assertEqual(
            predicate.logLevelForNamespace(
                "twext.web2.dav"), predicate.defaultLogLevel
        )
        self.assertEqual(
