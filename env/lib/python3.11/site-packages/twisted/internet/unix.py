...
class Port(_UNIXPort, tcp.Port):
    ...
    def __init__(
        ...
        tcp.Port.__init__(
            self, self._buildAddr(
                fileName).name, factory, backlog, reactor=reactor
        )
        self.mode = mode
        ...
        if self.wantPID:
            ...
            if not self.lockFile.lock():
                raise error.CannotListenError(
                    None, self.port, "Cannot acquire lock")
            else:
...
class Client(_SendmsgMixin, tcp.BaseClient):
    ...
    def __init__(self, filename, connector, reactor=None, checkPID=0):
        ...
        self._finishInit(
            self.doConnect, self.createInternetSocket(), None, reactor)
...
class ConnectedDatagramPort(DatagramPort):
    ...
    def __init__(
        ...
        DatagramPort.__init__(self, bindAddress, proto,
                              maxPacketSize, mode, reactor)
