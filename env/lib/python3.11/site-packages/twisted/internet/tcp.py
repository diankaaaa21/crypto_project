...
try:
    ...
    # pyOpenSSL 0.10
    from twisted.internet._newtls import ClientMixin as _TLSClientMixin
    from twisted.internet._newtls import ConnectionMixin as _TLSConnectionMixin
    from twisted.internet._newtls import ServerMixin as _TLSServerMixin
...
class Connection(
    ...
    def setTcpNoDelay(self, enabled):
        try:
            ...
            # continue.
            self.socket.setsockopt(
                socket.IPPROTO_TCP, socket.TCP_NODELAY, enabled)
    ...
    def doConnect(self):
        ...
        if err:
            self.failIfNotConnected(
                error.getConnectError((err, strerror(err))))
            return
...
if platformType == "win32":
    ...
    _reservedFD = _FileDescriptorReservation(
        lambda: open(os.devnull))  # type: ignore[assignment]
    ...
    def doRead(self):
        ...
        try:
            ...
            with _BuffersLogs(
                ...
                accepted = 0
                clients = _accept(
                    bufferingLogger, range(
                        numAccepts), self.socket, _reservedFD
                )
...
class Connector(base.BaseConnector):
    ...
    _addressType: type[address.IPv4Address |
