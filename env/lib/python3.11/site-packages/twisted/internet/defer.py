...
from contextvars import Context as _Context
...
class Deferred(Awaitable[_SelfResultT]):
    ...
    def addBoth(
        self,
        callback: Callable[
            Concatenate[Union[_SelfResultT, Failure],
                        _P], Union[Failure, _NextResultT]
        ],
        *args: _P.args,
        **kwargs: _P.kwargs,
    ) -> Deferred[_NextResultT]: ...
        ...
        while chain:
            ...
            while current.callbacks:
                ...
                if callback is _CONTINUE:
                    ...
                    chainee: Deferred[object] = args[0]
                ...
                try:
                    ...
                    if type(current.result) in _DEFERRED_SUBCLASSES:
                        ...
                        resultResult = getattr(
                            currentResult, "result", _NO_RESULT)
                        if (
                            ...
                            # using addCallbacks.
                            currentResult.callbacks.append(
                                current._continuation())
                            break
                        else:
                            # Yep, it did.  Steal it.
    ...
    if type(coro) in _DEFERRED_SUBCLASSES:
        ...
        try:
            ...
            raise NotACoroutineError(
                f"{coro!r} is not a coroutine or a Deferred")
    ...
    def _cbDeferred(
        ...
        if not self.called:
            if succeeded == SUCCESS and self.fireOnOneCallback:
                ...
                self.callback(
                    cast(_DeferredListResultListT[Any], self.resultList))
...
def race(ds: Sequence[Deferred[_T]]) -> Deferred[tuple[int, _T]]:
    ...
    for index, d in enumerate(ds):
        ...
        d.addCallbacks(succeeded, failed, callbackArgs=(
            index,), errbackArgs=(index,))
...
def _inlineCallbacks(
    ...
    while 1:
        ...
        if isDeferred:
            ...
            result.addBoth(_gotResultInlineCallbacks, waiting,
                           gen, status, context)  # type: ignore[attr-defined]
            if waiting[0]:
                # Haven't called back yet, set flag so that we get reinvoked
                # and return from the loop
                waiting[0] = False
...
def _handleCancelInlineCallbacks(
    ...
    status.deferred = Deferred(
        lambda d: _addCancelCallbackToDeferred(d, status))
