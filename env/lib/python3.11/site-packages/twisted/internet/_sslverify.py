    ...
    def __setattr__(self, attr, value):
        if attr not in _x509names:
            raise AttributeError(
                f"{attr} is not a valid OpenSSL X509 name field")
...
def _handleattrhelper(Class, transport, methodName):
    ...
    method = getattr(transport.getHandle(),
                     f"get_{methodName}_certificate", None)
    if method is None:
...
class CertificateRequest(CertBase):
    ...
    def load(Class, requestData, requestFormat=crypto.FILETYPE_ASN1):
        ...
        if not req.verify(req.get_pubkey()):
            raise VerifyError(
                f"Can't verify that request for {dn!r} is self-signed.")
        return Class(req)
...
class KeyPair(PublicKey):
    ...
    def selfSignedCert(self, serialNumber, **kw):
        ...
        return PrivateCertificate.fromCertificateAndKeyPair(
            self.signRequestObject(
                dn, self.requestObject(dn), serialNumber), self
        )
...
class ClientTLSOptions:
    ...
    def __init__(self, hostname, ctx):
        ...
        ctx.set_info_callback(_tolerateErrors(
            self._identityVerifyingInfoCallback))
