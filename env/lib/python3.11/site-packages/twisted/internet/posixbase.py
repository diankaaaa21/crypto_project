...
class PosixReactorBase(_DisconnectSelectableMixin, ReactorBase):
    ...
    def spawnProcess(
        ...
        if platformType == "posix":
            ...
            if uid is not None:
                raise ValueError(
                    "Setting UID is unsupported on this platform.")
            if gid is not None:
                raise ValueError(
                    "Setting GID is unsupported on this platform.")
            if usePTY:
                raise ValueError(
                    "The usePTY parameter is not supported on Windows.")
            if childFDs:
                raise ValueError(
                    "Customizing childFDs is not supported on Windows.")
    ...
    def listenSSL(self, port, factory, contextFactory, backlog=50, interface=""):
        if tls is not None:
            tlsFactory = tls.TLSMemoryBIOFactory(
                contextFactory, False, factory)
            port = self.listenTCP(port, tlsFactory, backlog, interface)
            port._type = "TLS"
            return port
        elif ssl is not None:
            p = ssl.Port(port, factory, contextFactory,
