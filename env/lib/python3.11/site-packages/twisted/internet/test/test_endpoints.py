...
try:
    ...
    from OpenSSL.SSL import Context as ContextType
    from OpenSSL.SSL import OP_NO_SSLv3, TLSv1_2_METHOD
...
class WrappingFactoryTests(unittest.TestCase):
    ...
    def test_failedBuildProtocol(self):
        ...
        d.addCallback(
            lambda e: self.assertEqual(
                e.args, ("My protocol is poorly defined.",))
        )
    ...
    def test_wrappingProtocolFileDescriptorReceiver(self):
        ...
        self.assertTrue(verifyObject(
            interfaces.IFileDescriptorReceiver, wrapper))
...
class ProcessEndpointTransportTests(unittest.TestCase):
    ...
    def setUp(self):
        self.reactor = MemoryProcessReactor()
        self.endpoint = endpoints.ProcessEndpoint(
            self.reactor, b"/bin/executable")
...
class WrappedIProtocolTests(unittest.TestCase):
    ...
    def test_logStderr(self):
        ...
        self.assertIn("wrote stderr unhandled by",
    ...
    def test_processDone(self):
        ...
        self.assertEqual(
            wpp.protocol.reason.check(
                error.ConnectionDone), error.ConnectionDone
        )
...
class SimpleHostnameResolverTests(unittest.SynchronousTestCase):
    ...
    def test_interface(self):
        ...
        """
        self.assertTrue(verifyObject(
            interfaces.IHostnameResolver, self.resolver))
    ...
    def test_endpointConnectingCancelled(self):
        ...
        ep, ignoredArgs, address = self.createClientEndpoint(
            deterministicResolvingReactor(
                mreactor, ["127.0.0.1"]), clientFactory
        )
...
class HostnameEndpointsFasterConnectionTests(unittest.TestCase):
    ...
    def test_ignoreUnknownAddressTypes(self):
        ...
        (host, port, factory, timeout,
         bindAddress) = self.mreactor.tcpClients[1]
    ...
    def test_IPv4IsFaster(self):
        ...
        (host, port, factory, timeout,
         bindAddress) = self.mreactor.tcpClients[0]
...
class HostnameEndpointBindAddressTypes(unittest.TestCase):
    ...
    def setUp(self):
        self.drr = deterministicResolvingReactor(
            MemoryReactor(), ["127.0.0.1"])

    def test_bytes(self):
        ba = b"1.2.3.4"
        ep = endpoints.HostnameEndpoint(
            self.drr, b"example.com", 80, bindAddress=ba)
...
class ServerStringTests(unittest.TestCase):
    ...
    def test_sslChainLoads(self):
        ...
        # preserved.
        expectedChainCerts = [
            Certificate.loadPEM(casPath.child(
                "thing%d.pem" % (n,)).getContent())
            for n in [1, 2]
        ]
        cf = server._sslContextFactory
        self.assertEqual(
            cf.extraCertChain[0].digest(
                "sha1"), expectedChainCerts[0].digest("sha1")
        )
        self.assertEqual(
            cf.extraCertChain[1].digest(
                "sha1"), expectedChainCerts[1].digest("sha1")
        )
...
def addFakePlugin(testCase, dropinSource="fakeendpoint.py"):
    ...
    getModule(__name__).filePath.sibling(
        dropinSource).copyTo(fp.child(dropinSource))
...
class ClientStringTests(unittest.TestCase):
    ...
    def test_tcpDefaults(self):
        ...
        reactor = object()
        client = endpoints.clientFromString(
            reactor, "tcp:host=example.com:port=1234")
...
class SSLClientStringTests(unittest.TestCase):
    ...
    def test_unreadableCertificate(self):
        ...
        self.assertEqual(
            [Certificate(x) for x in endpoints._loadCAsFromDir(
                casPathClone)._caCerts],
...
class AdoptedStreamServerEndpointTests(ServerEndpointTestCaseMixin, unittest.TestCase):
    ...
    def _createStubbedAdoptedEndpoint(self, reactor, fileno, addressFamily):
        ...
        tests.
        """
        e = endpoints.AdoptedStreamServerEndpoint(
    ...
    def test_singleUse(self):
        ...
        d = self.assertFailure(endpoint.listen(
            object()), error.AlreadyListened)
    ...
    def test_indexAndNameMutuallyExclusive(self) -> None:
        ...
        with self.assertRaises(ValueError):
            parser.parseStreamServer(
                reactor, domain="INET", index=0, name="foo")
...
class WrapClientTLSParserTests(unittest.TestCase):
    ...
    def test_tls(self):
        ...
        serverOptions = CertificateOptions(
            ...
            extraCertChain=[Certificate.loadPEM(
                chainPath.getContent()).original],
        ...
        self.assertEqual(
            peerCertificate, Certificate.loadPEM(pemPath.getContent()))
...
class WrapClientTLSTests(unittest.TestCase):
    ...
    def test_noOpenSSL(self):
        ...
        """
        replaced = replacingGlobals(
            endpoints.wrapClientTLS, TLSMemoryBIOFactory=None)
        notImplemented = self.assertRaises(
            NotImplementedError, replaced, None, None)
