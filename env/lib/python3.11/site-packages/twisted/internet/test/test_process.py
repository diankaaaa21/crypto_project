...
class ProcessTestsBuilderBase(ReactorBuilder):
    ...
    def test_writeToChildBadFileDescriptor(self):
        ...
        def write(transport, bytesToSend):
            try:
                self.assertRaises(
                    KeyError, transport.writeToChild, 13, bytesToSend)
            finally:
    ...
    def test_systemCallUninterruptedByChildExit(self):
        ...
        def f():
            try:
                ...
                f2 = subprocess.Popen(
                    [exe, "-c",
                        ("import time; time.sleep(0.5);" "print('Foo')")],
...
sys.stdout.flush()"""
        ...
        self.assertEqual(
            reportedChildFDs.intersection(
                set(stdFDs + [unlikelyFD])), set(stdFDs)
        )
...
class GetFileActionsTests(SynchronousTestCase):
    ...
    def test_closeNoCloexec(self) -> None:
        ...
        should be closed.
        """
        self.assertEqual(_getFileActions(
            [(0, False)], {}, CLOSE, DUP2), [(CLOSE, 0)])
