...
class TerminalBuffer(protocol.Protocol):
    ...
    def eraseToDisplayEnd(self):
        ...
        self.lines[self.y +
                   1:] = [self._emptyLine(self.width) for i in range(height)]

    def eraseToDisplayBeginning(self):
        self.eraseToLineBeginning()
        self.lines[: self.y] = [self._emptyLine(
            self.width) for i in range(self.y)]
    ...
    def insertLine(self, n=1):
        self.lines[self.y: self.y] = [
            self._emptyLine(self.width) for i in range(n)]
