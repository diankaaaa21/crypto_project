...
class StdioClient(basic.LineReceiver):
    ...
    def _printFailure(self, f):
        log.msg(f)
        e = f.trap(NotImplementedError,
                   filetransfer.SFTPError, OSError, IOError)
    ...
    def cmd_LN(self, rest):
        ...
        linkpath, targetpath = map(
            lambda x: os.path.join(self.currentDirectory,
                                   x), (linkpath, targetpath)
        )
...
"""
    ...
    def _cbReadFile(self, files, matchedFiles, directory, glob):
        if not isinstance(files, failure.Failure):
            if glob:
                ...
                matchedFiles.extend(
                    [f for f in files if fnmatch.fnmatch(f[0], glob)])
            else:
    ...
    def _printProgressBar(self, f, startTime):
        ...
        total = f.total
        try:
            winSize = struct.unpack("4H", fcntl.ioctl(
                0, tty.TIOCGWINSZ, "12345679"))
        except OSError:
            winSize = [None, 80]
        if diff == 0.0:
            speed = 0.0
