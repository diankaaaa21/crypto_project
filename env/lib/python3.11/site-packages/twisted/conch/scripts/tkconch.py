...
class TkConchMenu(Tkinter.Frame):
    def __init__(self, *args, **params):
        ...
        Tkinter.Label(self, text="Port Forwarding").grid(
        ...
        Tkinter.Button(self, text="Add", command=self.addForward).grid(
            column=1, row=7)
        ...
        if escape:
            if escape == "none":
                ...
                tkMessageBox.showerror(
                    "TkConch", "Bad escape character '%s'." % escape)
                finished = 0
    ...
    def verifyHostKey(self, pubKey, fingerprint):
        ...
        if goodKey == 1:  # good key
            ...
            if options["host"] == self.transport.getPeer().host:
                ...
                host = "{} ({})".format(
                    options["host"], self.transport.getPeer().host)
                khHost = "{},{}".format(
                    options["host"], self.transport.getPeer().host)
