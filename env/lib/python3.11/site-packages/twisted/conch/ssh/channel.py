...
class SSHChannel(log.Logger):
    ...
    _log = Logger()
    # type: ignore[assignment]  # only needed for client channels
    name: bytes = None
    ...
    def requestReceived(self, requestType, data):
        ...
        if f:
            return f(data)
        self._log.info(
            "unhandled request for {requestType}", requestType=requestType)
        return 0
    ...
    def writeExtended(self, dataType, data):
        ...
        while len(data) > self.remoteMaxPacket:
            self.conn.sendExtendedData(
                self, dataType, data[: self.remoteMaxPacket])
