...
class SSHConnection(service.SSHService):
    ...
    def ssh_CHANNEL_OPEN(self, packet):
        ...
        try:
            channel = self.getChannel(
                channelType, windowSize, maxPacket, packet)
            ...
            self.transport.sendPacket(
                MSG_CHANNEL_OPEN_CONFIRMATION, openConfirmPacket)
    ...
    def sendRequest(self, channel, requestType, data, wantReply=0):
        ...
        if channel.localClosed:
            return
        self._log.debug(
            "sending request {requestType}", requestType=requestType)
        self.transport.sendPacket(
            MSG_CHANNEL_REQUEST,
    ...
    def sendData(self, channel, data):
        ...
        self.transport.sendPacket(
            MSG_CHANNEL_DATA,
            struct.pack(
                ">L", self.channelsToRemoteChannel[channel]) + common.NS(data),
        )
    ...
    def sendEOF(self, channel):
        ...
        self.transport.sendPacket(
            MSG_CHANNEL_EOF, struct.pack(
                ">L", self.channelsToRemoteChannel[channel])
        )
    ...
    def getChannel(self, channelType, windowSize, maxPacket, data):
        ...
        """
        self._log.debug(
            "got channel {channelType!r} request", channelType=channelType)
        ...
        if chan is None:
            raise error.ConchError(
                "unknown channel", OPEN_UNKNOWN_CHANNEL_TYPE)
        else:
            chan.conn = self
            return chan
    ...
    def gotGlobalRequest(self, requestType, data):
        ...
        """
        self._log.debug(
            "got global {requestType} request", requestType=requestType)
        if hasattr(self.transport, "avatar"):  # this is a server!
