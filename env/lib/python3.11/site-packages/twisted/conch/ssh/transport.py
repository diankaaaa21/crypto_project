...
class SSHTransportBase(protocol.Protocol):
    ...
    supportedPublicKeys += [b"rsa-sha2-512",
                            b"rsa-sha2-256", b"ssh-rsa", b"ssh-dss"]
    ...
    def connectionMade(self):
        ...
        self.currentEncryptions = SSHCiphers(
    ...
    def getPacket(self):
        ...
        if self.incomingCompression:
            try:
                ...
                self.sendDisconnect(
                    DISCONNECT_COMPRESSION_ERROR, b"compression error")
                return
    ...
    def ssh_KEXINIT(self, packet):
        ...
        client = (kexAlgs, keyAlgs, outs[0], ins[0],
                  outs[1], ins[1], outs[2], ins[2])
        if self.isClient:
    ...
    def sendDebug(self, message, alwaysDisplay=False, language=b""):
        ...
        """
        self.sendPacket(
            MSG_DEBUG, (b"\1" if alwaysDisplay else b"\0") +
    ...
    def sendDisconnect(self, reason, desc):
        ...
        @type desc: L{str}
        """
        self.sendPacket(MSG_DISCONNECT, struct.pack(
            ">L", reason) + NS(desc) + NS(b""))
        self._log.info(
    ...
    def _keySetup(self, sharedSecret, exchangeHash):
        ...
        if self.isClient:  # Reverse for the client
            outs, ins = ins, outs
        self.nextEncryptions.setKeys(
            outs[0], outs[1], ins[0], ins[1], outs[2], ins[2])
    ...
    def receiveUnimplemented(self, seqnum):
        ...
        """
        self._log.warn(
            "other side unimplemented packet #{seqnum}", seqnum=seqnum)
...
class SSHClientTransport(SSHTransportBase):
    ...
    def ssh_KEXINIT(self, packet):
        ...
        if _kex.isEllipticCurve(self.kexAlg):
            ...
            self.sendPacket(
                MSG_KEX_DH_GEX_REQUEST_OLD, NS(
                    self._encodeECPublicKey(self.ecPub))
            )
    ...
    def _ssh_KEX_ECDH_REPLY(self, packet):
        ...
        def _continue_KEX_ECDH_REPLY(ignored, hostKey, pubKey, signature):
            ...
            if not keys.Key.fromString(theirECHost).verify(signature, exchangeHash):
                self.sendDisconnect(
                    DISCONNECT_KEY_EXCHANGE_FAILED, b"bad signature")
            else:
                self._keySetup(sharedSecret, exchangeHash)
