...
class SSHUserAuthServer(service.SSHService):
    ...
    def tryAuth(self, kind, user, data):
        ...
        if f:
            ...
            if not ret:
                return defer.fail(
                    error.ConchError(
                        f"{kind} return None instead of a Deferred")
                )
            else:
                return ret
    ...
    def ssh_USERAUTH_REQUEST(self, packet):
        ...
        if not d:
            self._ebBadAuth(failure.Failure(
                error.ConchError("auth returned none")))
            return
    ...
    def _cbFinishedAuth(self, result):
        ...
        service = self.transport.factory.getService(
            self.transport, self.nextService)
        if not service:
            raise error.ConchError(
                f"could not get next service: {self.nextService}")
        self._log.debug(
    ...
    def _ebMaybeBadAuth(self, reason):
        ...
        self.transport.sendPacket(
            MSG_USERAUTH_FAILURE, NS(
                b",".join(self.supportedAuthentications)) + b"\xff"
        )
    ...
    def _ebBadAuth(self, reason):
        ...
        self.transport.sendPacket(
            MSG_USERAUTH_FAILURE, NS(
                b",".join(self.supportedAuthentications)) + b"\x00"
        )
...
class SSHUserAuthClient(service.SSHService):
    ...
    def ssh_USERAUTH_PK_OK(self, packet):
        ...
        func = getattr(
            self,
            "ssh_USERAUTH_PK_OK_%s" % nativeString(
                self.lastAuth.replace(b"-", b"_")),
            None,
        )
        if func is not None:
            return func(packet)
    ...
    def _cbGetPublicKey(self, publicKey):
        ...
        if publicKey is not None:
            ...
            self._log.debug(
                "using key of type {keyType}", keyType=publicKey.type())
            self.askForAuth(
                b"publickey", b"\x00" +
                NS(publicKey.sshType()) + NS(publicKey.blob())
            )
            return True
        else:
            return False
