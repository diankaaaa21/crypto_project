...
class Key:
        ...
        if keyType == b"ssh-dss":
            ...
            return cls(
                dsa.DSAPublicNumbers(
                    y=y, parameter_numbers=dsa.DSAParameterNumbers(
                        p=p, q=q, g=g)
    ...
    def _fromPrivateOpenSSH_v1(cls, data, passphrase):
        ...
        if cipher != b"none":
            ...
            privKeyList = decryptor.update(
                encPrivKeyList) + decryptor.finalize()
        else:
            if kdf != b"none":
                raise BadKeyError(
        ...
        if check1 != check2:
            raise BadKeyError(
                "check values do not match: %d != %d" % (check1, check2))
    ...
    def fingerprint(self, format=FingerprintFormats.MD5_HEX):
        ...
        if format is FingerprintFormats.SHA256_BASE64:
            ...
            return nativeString(
                b":".join(
                    [binascii.hexlify(x) for x in iterbytes(
                        md5(self.blob()).digest())]
                )
            )
        else:
            raise BadFingerPrintFormat(
                f"Unsupported fingerprint format: {format}")
    ...
    def type(self) -> Literal["RSA", "DSA", "EC", "Ed25519"]:
        ...
        if isinstance(self._keyObject, (rsa.RSAPublicKey, rsa.RSAPrivateKey)):
            ...
            self._keyObject, (ed25519.Ed25519PublicKey,
    ...
    def _toPrivateOpenSSH_v1(self, comment=None, passphrase=None):
        ...
        if passphrase:
            ...
            encPrivKeyList = encryptor.update(
                privKeyList) + encryptor.finalize()
        else:
    ...
    def _toString_AGENTV3(self, **kwargs):
        ...
        if not self.isPublic():
            if self.type() == "RSA":
                ...
                values = (data["p"], data["q"],
                          data["g"], data["y"], data["x"])
