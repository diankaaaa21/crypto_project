...
class ListingTests(TestCase):
    ...
    def setUp(self):
        ...
        # it was before.
        if "TZ" in os.environ:
            self.addCleanup(operator.setitem, os.environ,
                            "TZ", os.environ["TZ"])
...
class StdioClientTests(TestCase):
    ...
    def test_cmd_PUTMultipleNoRemotePath(self):
        ...
        self.fakeFilesystem.put(firstRemotePath, flags,
    ...
    def test_cmd_PUTMultipleWithRemotePath(self):
        ...
        deferred = self.client.cmd_PUT(
            "{} remote".format(os.path.join(parent, "*")))
        self.successResultOf(deferred)
...
class OurServerCmdLineClientTests(CFTPClientTestBase):
    ...
    def testList(self):
        ...
        def _check(results):
            self.assertEqual(
                results[0],
                [b"testDirectory", b"testRemoveFile",
            ...
            self.assertEqual(
                results[2], [b"testRemoveFile", b"testRenameFile"])
            self.assertEqual(
                results[3], [b".testHiddenFile",
    ...
    def testPut(self):
        ...
        def _checkPut(result):
            self.assertFilesEqual(
                self.testDir.child(
                    "testfile1"), self.testDir.child('test"file2')
            )
            self.assertTrue(result.endswith(expectedOutput))
            return self.runCommand('rm "test\\"file2"')

        d = self.runCommand(
            f'put {self.testDir.path}/testfile1 "test\\"file2"')
    ...
    def test_putOverLongerFile(self):
        ...
        def _checkPut(result):
            self.assertFilesEqual(
                self.testDir.child(
                    "shorterFile"), self.testDir.child("longerFile")
            )
    ...
    def test_putMultipleOverLongerFile(self):
        ...
        def _checkPut(result):
            self.assertFilesEqual(someDir.child(
                "file"), self.testDir.child("file"))
