...
class HelperTests(TestCase):
    ...
    def test_pwdGetByName(self):
        ...
        """
        userdb = UserDatabase()
        userdb.addUser("alice", "secrit", 1, 2,
                       "first last", "/foo", "/bin/sh")
        self.patch(checkers, "pwd", userdb)
        self.assertEqual(checkers._pwdGetByName(
            "alice"), userdb.getpwnam("alice"))
    ...
    def test_shadowGetByName(self):
        ...
        self.assertEqual(checkers._shadowGetByName(
            "bob"), userdb.getspnam("bob"))
...
class SSHProtocolCheckerTests(TestCase):
    ...
    def test_registerCheckerWithInterface(self):
        ...
        self.assertEqual(checker.credentialInterfaces, [])
        checker.registerChecker(
            checkers.SSHPublicKeyDatabase(), IUsernamePassword)
...
class UNIXPasswordDatabaseTests(TestCase):
    ...
    def test_defaultCheckers(self):
        ...
        pwd.addUser(
            "alice", crypted(
                "alice", "password"), 1, 2, "foo", "/foo", "/bin/sh"
        )
    ...
    def test_passInCheckers(self):
        ...
        self.assertLoggedIn(
            checker.requestAvatarId(UsernamePassword(
                b"anybody", b"secret")), b"anybody"
        )
...
class UNIXAuthorizedKeysFilesTests(TestCase):
    ...
    def test_noKeysForUnauthorizedUser(self):
        ...
        """
        keydb = checkers.UNIXAuthorizedKeysFiles(
            self.userdb, parseKey=lambda x: x)
        self.assertEqual([], list(keydb.getAuthorizedKeys(b"bob")))
    ...
    def test_allKeysInAllAuthorizedFilesForAuthorizedUser(self):
        ...
        self.assertEqual(
            self.expectedKeys +
            [b"key 3"], list(keydb.getAuthorizedKeys(b"alice"))
        )
    ...
    def test_ignoresNonexistantFile(self):
        ...
        keydb = checkers.UNIXAuthorizedKeysFiles(
            self.userdb, parseKey=lambda x: x)
        self.assertEqual(self.expectedKeys, list(
            keydb.getAuthorizedKeys(b"alice")))
