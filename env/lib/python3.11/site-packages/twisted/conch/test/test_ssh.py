        ...
        try:
            listener = reactor.listenTCP(
                ...
                forwarding.SSHListenForwardingFactory(
                    self.conn, (host,
                                port), forwarding.SSHListenServerForwardingChannel
                ),
                interface=host,
            )
...
class FalseTransport:
    ...
    def loseConnection(self):
        ...
        self.proto.processEnded(failure.Failure(
            ProcessTerminated(255, None, None)))
...
if cryptography is not None:
    ...
    class ConchTestClient(ConchTestBase, transport.SSHClientTransport):
        ...
        def connectionSecure(self):
            self.requestService(
                ConchTestClientAuth(
                    b"testuser", ConchTestClientConnection(
                        self._channelFactory)
                )
            )
    ...
    def test_shell(self):
        ...
        def cbExited(ignored):
            if self.channel.status != 0:
                log.msg("shell exit status was not 0: %i" %
                        (self.channel.status,))
            self.assertEqual(
...
class SSHFactoryTests(unittest.TestCase):
    ...
    def test_buildProtocolWithPrimes(self):
        ...
        self.assertIn(b"diffie-hellman-group-exchange-sha1",
                      p2.supportedKeyExchanges)
        self.assertIn(b"diffie-hellman-group-exchange-sha256",
                      p2.supportedKeyExchanges)
...
class MPTests(unittest.TestCase):
    ...
    def test_getMP(self):
        ...
        """
        self.assertEqual(self.getMP(
            b"\x00\x00\x00\x04\x00\x00\x00\x01"), (1, b""))
