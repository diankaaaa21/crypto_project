...
class ConchTestForwardingProcess(protocol.ProcessProtocol):
    ...
    def _connect(self):
        ...
        cc = protocol.ClientCreator(
            reactor, ConchTestForwardingPort, self, self.data)
...
class ConchServerSetupMixin:
    ...
    def setUp(self):
        ...
        if HAS_IPV6:
            self.echoServerV6 = reactor.listenTCP(
                0, EchoFactory(), interface="::1")
    ...
    def tearDown(self):
        ...
        if HAS_IPV6:
            deferreds.append(defer.maybeDeferred(
                self.echoServerV6.stopListening))
...
class CmdLineClientTests(ForwardingMixin, TestCase):
    ...
    def execute(self, remoteCommand, process, sshArgs="", conchArgs=None):
        ...
        for var in env:
            ...
            encodedEnv[var] = val
        reactor.spawnProcess(process, sys.executable,
                             encodedCmds, env=encodedEnv)
        return process.deferred
