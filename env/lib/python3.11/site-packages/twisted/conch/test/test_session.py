...
class StubConnection:
    ...
    def sendRequest(self, channel, request, data, wantReply=False):
        ...
        if self.closes.get(channel):
            return
        self.requests.setdefault(channel, []).append(
            (request, data, wantReply))
        if wantReply:
...
class SessionInterfaceTests(RegistryUsingMixin, TestCase):
    ...
    def test_client_closed_with_env_subsystem(self):
        ...
        self.assertTrue(
            self.session.requestReceived(
                b"env", common.NS(b"FOO") + common.NS(b"bar"))
        )
        self.assertTrue(
            self.session.requestReceived(
    ...
    def test_lookupSubsystemDoesNotNeedISession(self):
        ...
        s = session.SSHSession(
            avatar=SubsystemOnlyAvatar(), conn=StubConnection())
    ...
    def test_requestShellWithData(self):
        ...
        self.assertEqual(
            self.session.session.shellTransport.data, b"some data\x00")
        self.assertEqual(
    ...
    def test_requestPty(self):
        ...
        self.assertEqual(test_session.session.ptyRequest,
                         (b"good", (1, 2, 3, 4), []))
    ...
    def test_setEnv(self):
        ...
        self.assertIsInstance(self.session.session,
                              StubSessionForStubAvatarWithEnv)
        self.assertRequestRaisedRuntimeError()
    ...
    def test_requestWindowChange(self):
        ...
        self.assertTrue(
            self.session.requestReceived(
                b"window_change", session.packRequest_window_change(
                    (1, 2, 3, 4))
            )
        )
...
class SSHSessionProcessProtocolTests(TestCase):
    ...
    def assertRequestsEqual(self, expectedRequests):
        ...
        """
        self.assertEqual(
            self.session.conn.requests[self.session], expectedRequests)
    ...
    def test_getSignalName(self):
        ...
        for signalName in session.SUPPORTED_SIGNALS:
            ...
            self.assertEqual(
                sshName, signalName, "%i: %s != %s" % (
                    signalValue, sshName, signalName)
            )
