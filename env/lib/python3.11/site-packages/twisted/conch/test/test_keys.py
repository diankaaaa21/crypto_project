...
class KeyTests(unittest.TestCase):
    ...
    def test__guessStringType(self):
        ...
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.publicRSA_openssh), "public_openssh"
        )
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.publicDSA_openssh), "public_openssh"
        )
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.publicECDSA_openssh), "public_openssh"
        )
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.publicSKECDSA_openssh), "public_openssh"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.privateRSA_openssh_new), "private_openssh"
        )
        self.assertEqual(
            keys.Key._guessStringType(
        ...
        self.assertEqual(
            keys.Key._guessStringType(
                keydata.privateDSA_openssh_new), "private_openssh"
        )
        self.assertEqual(
            keys.Key._guessStringType(
        ...
        self.assertEqual(keys.Key._guessStringType(
            keydata.publicDSA_lsh), "public_lsh")
        self.assertEqual(
    ...
    def test_public(self):
        ...
        if ED25519_SUPPORTED:
            privateEd25519Key = keys.Key.fromString(
                keydata.privateEd25519_openssh_new)
            publicEd25519Key = keys.Key.fromString(
                keydata.publicEd25519_openssh)
            self.assertEqual(privateEd25519Key.public(),
                             publicEd25519Key.public())
    ...
    def test_isPublic(self):
        ...
        if ED25519_SUPPORTED:
            ed25519Key = keys.Key.fromString(
                keydata.privateEd25519_openssh_new)
...
-----END DSA PRIVATE KEY-----"""
    ...
    def test_fromOpenSSH_v1_format(self):
        ...
        for old, new in (
            ...
            self.assertEqual(keys.Key.fromString(new),
                             keys.Key.fromString(old))
        self.assertEqual(
            keys.Key.fromString(
...
-----END DSA PRIVATE KEY-----"""
    ...
    def test_fromStringNormalizesUnicodePassphrase(self):
        ...
        key_data = key.toString(
            "openssh", passphrase="verschl\u00FCsselt".encode())
        self.assertEqual(
            keys.Key.fromString(
                key_data, passphrase="verschlu\u0308sselt"), key
        )
    ...
    def test_fromStringErrors(self):
        ...
        self.assertRaises(keys.BadKeyError,
                          keys.Key.fromString, b"", "bad_type")
...
-----END RSA PRIVATE KEY-----""",
    ...
    def test_fromFile(self):
        ...
        """
        self.assertEqual(
            keys.Key.fromFile(self.keyFile), keys.Key.fromString(
                keydata.privateRSA_lsh)
        )
    ...
    def test_type(self):
        ...
        if ED25519_SUPPORTED:
            ...
            self.assertEqual(
                keys.Key(self.ed25519Obj).sshType(), b"ssh-ed25519")
    ...
    def test_supportedSignatureAlgorithms(self):
        ...
        self.assertRaises(RuntimeError, keys.Key(
            None).supportedSignatureAlgorithms)
        self.assertRaises(RuntimeError, keys.Key(
            self).supportedSignatureAlgorithms)
    ...
    def test_fromBlobEd25519(self):
        ...
        ed25519Blob = common.NS(b"ssh-ed25519") + \
            common.NS(ed25519PublicData["a"])
    ...
    def test_fromPrivateBlobUnsupportedType(self):
        ...
        self.assertRaises(keys.BadKeyError,
                          keys.Key._fromString_PRIVATE_BLOB, badBlob)
    ...
    def test_toOpenSSHRSA(self):
        ...
        self.assertEqual(key.toString("openssh").strip(),
                         keydata.privateRSA_openssh)
        self.assertTrue(
            key.toString("openssh", passphrase=b"encrypted").find(
                b"DEK-Info") > 0
        )
        self.assertEqual(
    ...
    def test_toOpenSSHRSA_v1_format(self):
        ...
        self.assertEqual(
            b"-----BEGIN OPENSSH PRIVATE KEY-----", new_key_data.splitlines()[
                0]
        )
        self.assertEqual(
            b"-----BEGIN OPENSSH PRIVATE KEY-----", new_enc_key_data.splitlines()[
                0]
        )
    ...
    def test_toStringNormalizesUnicodePassphrase(self):
        ...
        self.assertEqual(
            keys.Key.fromString(
                key_data, passphrase="verschl\u00FCsselt".encode()),
            key,
        )
        # U+FFFF is a "noncharacter" and guaranteed to have General_Category
        # Cn (Unassigned).
    ...
    def test_toStringErrors(self):
        ...
        format type.
        """
        self.assertRaises(keys.BadKeyError, keys.Key(
            self.rsaObj).toString, "bad_type")
    ...
    def test_signAndVerifyRSA(self):
        ...
        self.assertIsNone(
            key._keyObject.public_key().verify(
                common.getNS(signature)[
                    0], data, padding.PKCS1v15(), hashes.SHA1()
            )
        )
    ...
    def test_signAndVerifyRSASHA256(self):
        ...
        self.assertIsNone(
            key._keyObject.public_key().verify(
                common.getNS(signature)[
                    0], data, padding.PKCS1v15(), hashes.SHA256()
            )
        )
    ...
    def test_signAndVerifyRSASHA512(self):
        ...
        self.assertIsNone(
            key._keyObject.public_key().verify(
                common.getNS(signature)[
                    0], data, padding.PKCS1v15(), hashes.SHA512()
            )
        )
