...
class DomainDeliveryBase:
    ...
    def receivedHeader(self, helo, origin, recipients):
        ...
        fromUser = b"from " + helo[0] + \
            b" ([" + helo[1] + b"]" + heloStr + authStr
        by = (
            b"by "
            + self.host
            + b" with "
        ...
        forUser = (
            b"for <" + b" ".join(map(bytes, recipients)) +
            b"> " + smtp.rfc822date()
        )
    ...
    def validateFrom(self, helo, origin):
        ...
        if not helo:
            raise smtp.SMTPBadSender(
                origin, 503, "Who are you?  Say HELO first.")
        if origin.local != b"" and origin.domain == b"":
            raise smtp.SMTPBadSender(
                origin, 501, "Sender address must contain domain.")
        return origin
        ...
        try:
            ...
            return portal.login(
                pop3.APOPCredentials(
                    self.magic, user, digest), None, pop3.IMailbox
            )
