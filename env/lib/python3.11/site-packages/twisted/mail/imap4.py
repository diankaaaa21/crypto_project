...
from twisted.mail.interfaces import IAccountIMAP as IAccount
from twisted.mail.interfaces import IClientAuthentication
from twisted.mail.interfaces import ICloseableMailboxIMAP as ICloseableMailbox
from twisted.mail.interfaces import IMailboxIMAP as IMailbox
from twisted.mail.interfaces import IMailboxIMAPInfo as IMailboxInfo
from twisted.mail.interfaces import IMailboxIMAPListener as IMailboxListener
from twisted.mail.interfaces import IMessageIMAP as IMessage
from twisted.mail.interfaces import IMessageIMAPCopier as IMessageCopier
from twisted.mail.interfaces import IMessageIMAPFile as IMessageFile
from twisted.mail.interfaces import IMessageIMAPPart as IMessagePart
from twisted.mail.interfaces import INamespacePresenter
from twisted.mail.interfaces import ISearchableIMAPMailbox as ISearchableMailbox
...
class MessageSet:
    ...
    def add(self, start, end=_empty):
        ...
        start, end = sorted(
            [start, end], key=functools.partial(
                _swap, that=self._infinity, ifIs=None)
        )
    ...
    def clean(self):
        ...
        ranges = sorted(_swapAllPairs(
            self.ranges, that=self._infinity, ifIs=None))
        ...
        self.ranges = _swapAllPairs(
            mergedRanges[1:], that=None, ifIs=self._infinity)
...
class IMAP4Server(basic.LineReceiver, policies.TimeoutMixin):
    ...
    def connectionMade(self):
        self.tags = {}
        self.canStartTLS = interfaces.ITLSTransport(
            self.transport, None) is not None
    ...
    def lineReceived(self, line):
        ...
        try:
            ...
            self.sendUntaggedResponse(
                b"BAD Server error: " + networkString(str(e)))
            log.err()
    ...
    def parse_command(self, line):
        ...
        try:
            ...
            self.sendBadResponse(
                tag, b"Illegal syntax: " + networkString(str(e)))
    ...
    def __doCommand(self, tag, handler, args, parseargs, line, uid):
        ...
        if line:
            ...
            raise IllegalClientResponse(
                "Too many arguments for command: " + repr(line))
    ...
    def sendServerGreeting(self):
        msg = b"[CAPABILITY " + \
            b" ".join(self.listCapabilities()) + b"] " + self.IDENT
    ...
    def do_CAPABILITY(self, tag):
        self.sendUntaggedResponse(
            b"CAPABILITY " + b" ".join(self.listCapabilities()))
    ...
    def __cbAuthChunk(self, result, chal, tag):
        ...
        if chal.moreChallenges():
            ...
            self.portal.login(chal, None, IAccount).addCallbacks(
                self.__cbAuthResp, self.__ebAuthResp, (
                    tag,), None, (tag,), None
            )
    ...
    def __ebAuthResp(self, failure, tag):
        if failure.check(UnauthorizedLogin):
            self.sendNegativeResponse(
                tag, b"Authentication failed: unauthorized")
            ...
            self.sendBadResponse(
                tag, b"Server error: login failed unexpectedly")
            log.err(failure)
    ...
    def __cbLogin(self, result, tag):
        ...
        if iface is not IAccount:
            self.sendBadResponse(
                tag, b"Server error: login returned unexpected value")
    ...
    def _ebListWork(self, failure, tag):
        self.sendBadResponse(
            tag, b"Server error encountered while listing mailboxes.")
        log.err(failure)
    ...
    def _ebStatusGotMailbox(self, failure, tag):
        self.sendBadResponse(
            tag, b"Server error encountered while opening mailbox.")
        log.err(failure)
    ...
    auth_APPEND = (do_APPEND, arg_astring, opt_plist,
                   opt_datetime, arg_literal)
    ...
    def do_CHECK(self, tag):
        ...
        if d is None:
            ...
            d.addCallbacks(
                self.__cbCheck, self.__ebCheck, callbackArgs=(
                    tag,), errbackArgs=(tag,)
            )
    ...
    def do_CLOSE(self, tag):
        ...
        if d is not None:
            d.addCallbacks(self.__cbClose, self.__ebClose,
                           (tag,), None, (tag,), None)
        else:
    ...
    def do_EXPUNGE(self, tag):
        if self.mbox.isWriteable():
            ...
            self.sendNegativeResponse(
                tag, b"EXPUNGE ignored on read-only mailbox")
    ...
    def search_OR(self, query, id, msg, lastIDs):
        ...
        """
        (lastSequenceId, lastMessageId) = lastIDs
        a = self._singleSearchStep(
            query, id, msg, lastSequenceId, lastMessageId)
        b = self._singleSearchStep(
            query, id, msg, lastSequenceId, lastMessageId)
        return a or b
    ...
    def spew_bodystructure(self, id, msg, _w=None, _f=None):
        _w(b"BODYSTRUCTURE " +
           collapseNestedLists([getBodyStructure(msg, True)]))
    ...
    def __cbCopied(self, deferredIds, tag, mbox):
        ...
        if failures:
            self.sendNegativeResponse(
                tag, "[ALERT] Some messages were not copied")
        else:
...
class IMAP4Client(basic.LineReceiver, policies.TimeoutMixin):
    ...
    def __cbLoginCaps(self, capabilities, username, password):
        ...
        tlsableTransport = interfaces.ITLSTransport(
            self.transport, None) is not None

        # If our transport is not already using TLS, we might want to try to switch to TLS.
        nontlsTransport = interfaces.ISSLTransport(
            self.transport, None) is None
    ...
    def select(self, mailbox):
        ...
        resp = ("FLAGS", "EXISTS", "RECENT", "UNSEEN",
                "PERMANENTFLAGS", "UIDVALIDITY")
        ...
        for split in lines:
            if len(split) > 0 and split[0].upper() == b"OK":
                ...
                if split[0].upper() == b"FLAGS":
                    datum["FLAGS"] = tuple(nativeString(flag)
                                           for flag in split[1])
    ...
    def status(self, mailbox, *names):
        ...
        try:
            ...
            raise ValueError(
                f"Unknown names: {set(names) - set(self._statusNames)!r}")
    ...
    def fetchSpecific(
        ...
        cmd = fmt % (messages, peek and ".PEEK" or "",
                     number, header, payload, extra)
    ...
    def _fetch(self, messages, useUID=0, **terms):
        ...
        cmd = messages + \
            b" (" + b" ".join([s.upper() for s in encodedTerms]) + b")"
    ...
    def _store(self, messages, cmd, silent, flags, uid):
        ...
        args = b" ".join(
            (messages, cmd, b"(" + b" ".join(encodedFlags) + b")"))
...
def collapseStrings(results):
    ...
    def pred(e): return isinstance(e, tuple)
    tran = {
...
def collapseNestedLists(items):
    ...
    for i in items:
        ...
        if i is None:
            ...
            if _needsLiteral(i):
                pieces.extend([b" ", b"{%d}" %
                              (len(i),), IMAP4Server.delimiter, i])
            else:
                pieces.extend([b" ", _quote(i)])
        elif hasattr(i, "read"):
            d = i.read()
            pieces.extend([b" ", b"{%d}" %
                          (len(d),), IMAP4Server.delimiter, d])
        else:
...
class MemoryAccountWithoutNamespaces:
    ...
    def delete(self, name):
        ...
        if len(self._inferiorNames(name)) > 1:
            raise MailboxException(
                f'Name "{name}" has inferior hierarchical names')
...
class _SinglepartMessageStructure(_MessageStructure):
    ...
    _HEADERS = ["content-id", "content-description",
...
class _MultipartMessageStructure(_MessageStructure):
    ...
    def encode(self, extended):
        ...
        """
        result = [_getMessageStructure(p).encode(extended)
...
class _FetchParser:
    class Envelope:
        ...
        type = "envelope"
        def __str__(self): return "envelope"

    class Flags:
        type = "flags"
        def __str__(self): return "flags"

    class InternalDate:
        type = "internaldate"
        def __str__(self): return "internaldate"

    class RFC822Header:
        type = "rfc822header"
        def __str__(self): return "rfc822.header"

    class RFC822Text:
        type = "rfc822text"
        def __str__(self): return "rfc822.text"

    class RFC822Size:
        type = "rfc822size"
        def __str__(self): return "rfc822.size"

    class RFC822:
        type = "rfc822"
        def __str__(self): return "rfc822"

    class UID:
        type = "uid"
        def __str__(self): return "uid"
        ...
        def getBytes(self, length: Optional[int] = None) -> bytes:
            ...
            if self.part:
                part = b".".join([str(x + 1).encode("ascii")
                                 for x in self.part])  # type: ignore[unreachable]
                separator = b"."
            #            if self.peek:
            #                base += '.PEEK'
            if self.header:
            ...
            if self.partialBegin is not None:
                # type: ignore[unreachable]
                if length is None or length > self.partialLength:
                    base += b"<%d.%d>" % (self.partialBegin,
                                          self.partialLength)
                else:
    ...
    class BodyStructure:
        type = "bodystructure"
        def __str__(self): return "bodystructure"
    ...
    class Header:
        ...
        def __bytes__(self) -> bytes:
            ...
            if self.part:
                ...
                # code?
                # type: ignore[unreachable]
                base = b".".join([(x + 1).__bytes__()
                                 for x in self.part]) + b"." + base
            return base
