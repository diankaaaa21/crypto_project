...
from twisted.mail._except import SMTPTLSError as TLSError
from twisted.mail._except import TLSRequiredError
from twisted.mail.interfaces import (
    IClientAuthentication,
    IMessageDelivery,
    IMessageDeliveryFactory,
)
from twisted.mail.interfaces import IMessageSMTP as IMessage
    ...
    def __init__(self, addr, defaultDomain=None):
        ...
        while atl:
            if atl[0] == b"<":
                ...
                raise AddressError(
                    f"Parse error at {atl[0]!r} of {(addr, atl)!r}")
            else:
                if not domain:
                    local.append(atl[0])
                else:
...
class SMTP(basic.LineOnlyReceiver, policies.TimeoutMixin):
    ...
    def do_HELO(self, rest):
        ...
        self._from = None
        self._to = []
        self.sendCode(250, self.host + b" Hello " +
                      host + b", nice to meet you")
    ...
    def do_RCPT(self, rest):
        ...
        d.addCallbacks(self._cbToValidate, self._ebToValidate,
                       callbackArgs=(user,))
    ...
    def _cbAnonymousAuthentication(self, result):
        ...
        if issubclass(iface, IMessageDeliveryFactory):
            ...
            raise RuntimeError(
                f"{iface.__name__} is not a supported interface")
    ...
    def receivedHeader(self, helo, origin, recipients):
        ...
        by = b"by %s with %s (%s)" % (
            domain, self.__class__.__name__, longversion)
...
class SMTPClient(basic.LineReceiver, policies.TimeoutMixin):
    ...
    def smtpState_msgSent(self, code, resp):
        if self._from is not None:
            self.sentMail(
                code, resp, len(
                    self.successAddresses), self.toAddressesResult, self.log
            )
        ...
        if code not in SUCCESS:
            ...
            for addr, acode, aresp in addresses:
                if acode not in SUCCESS:
                    errlog.append(
                        addr + b": " +
                        networkString("%03d" % (acode,)) + b" " + aresp
                    )
