...
class BounceTests(TestCase):
    ...
    def testRelay(self):
        self.assertEqual(self.domain.willRelay(
            "random q emailer", "protocol"), False)
...
class MaildirAppendStringTests(TestCase, _AppendTestMixin):
    ...
    def _cbTestAppend(self, ignored, mbox):
        ...
        self.assertEqual(
            [len(mbox.getMessage(i).read())
             for i in range(10)], list(range(1, 11))
        )
...
class MaildirDirdbmDomainTests(TestCase):
    ...
    def test_requestAvatarId(self):
        ...
        self.assertRaises(cred.error.UnauthorizedLogin,
    ...
    def test_userDirectory(self):
        ...
        self.assertEqual(
            self.D.userDirectory("nouser"), os.path.join(
                self.D.root, "postmaster")
        )
...
class ServiceDomainTests(TestCase):
    ...
    def testValidateFrom(self):
        ...
        self.assertRaises(smtp.SMTPBadSender,
...
class VirtualPOP3Tests(TestCase):
    ...
    def testAuthenticateIncorrectUserAPOP(self):
        ...
        return self.assertFailure(
            self.P.authenticateUserAPOP(
                "resu", resp), cred.error.UnauthorizedLogin
        )
    ...
    def testAuthenticateIncorrectResponseAPOP(self):
        ...
        return self.assertFailure(
            self.P.authenticateUserAPOP(
                "user", resp), cred.error.UnauthorizedLogin
        )
    ...
    def _lookup(self, name, cls, type, timeout=None):
        if name in self.addresses and type == dns.MX:
            ...
            for a in self.addresses[name]:
                hdr = dns.RRHeader(name, dns.MX, dns.IN,
                                   60, dns.Record_MX(0, a))
                results.append(hdr)
...
class MXTests(TestCase):
    ...
    def test_mailExchangePreference(self):
        ...
        records = [
            RRHeader(name=domain, type=Record_MX.TYPE,
    ...
    def test_badExchangeExpires(self):
        ...
        records = [
            RRHeader(
                name=domain, type=Record_MX.TYPE, payload=Record_MX(
                    0, previouslyBad)
            ),
            RRHeader(name=domain, type=Record_MX.TYPE,
                     payload=Record_MX(1, good)),
    ...
    def test_cnameChain(self):
        ...
        def cbChainTooLong(error):
            self.assertEqual(
                error.args[0], Record_CNAME(
                    "x-%d.example.com" % (cnameLimit + 1,))
            )
...
class LiveFireExerciseTests(TestCase):
    ...
    def testLocalDelivery(self):
        service = mail.mail.MailService()
        service.smtpPortal.registerChecker(
            cred.checkers.AllowAnonymousAccess())
        ...
        map(service.portals[""].registerChecker,
            domain.getCredentialsCheckers())
    ...
    def testRelayDelivery(self):
        ...
        insServ.smtpPortal.registerChecker(
            cred.checkers.AllowAnonymousAccess())
        ...
        destServ.smtpPortal.registerChecker(
            cred.checkers.AllowAnonymousAccess())
        domain = mail.maildir.MaildirDirdbmDomain(
            destServ, "destinationDomain")
        domain.addUser("user", "password")
...
class AliasTests(TestCase):
    ...
    def testHandle(self):
        ...
        self.assertEqual(
            result["multiuser"], ["first@host",
                                  "second@host", "last@anotherhost"]
        )
...
class AddressAliasTests(TestCase):
    ...
    def setUp(self):
        ...
        self.alias = mail.alias.AddressAlias(
            self.address, domains, self.address)
...
while read i; do
    ...
    def test_processAliasTimeout(self):
        ...
        def timedOut(ignored):
            ...
            # with it.
            proto.processEnded(ProcessTerminated(
                self.signalStatus(signal.SIGKILL)))
    ...
    def test_cyclicAlias(self):
        ...
        r.sort()
        expected = map(
            str, [mail.alias.MessageWrapper(DummyProcess(), "echo")])
        expected.sort()
