...
class MessageSetTests(SynchronousTestCase):
    ...
    def test_sequence_setExamples(self):
        ...
        """
        fromFifteenMessages = (
            MessageSet(2) + MessageSet(4, 7) +
            MessageSet(9) + MessageSet(12, None)
        )
        fromFifteenMessages.last = 15
        self.assertEqual(
            ",".join(str(i)
                     for i in fromFifteenMessages), "2,4,5,6,7,9,12,13,14,15"
        )

        fromTenMessages = MessageSet(None, 4) + MessageSet(5, 7)
        fromTenMessages.last = 10
        self.assertEqual(",".join(str(i)
                         for i in fromTenMessages), "4,5,6,7,8,9,10")
...
class IMAP4HelperTests(TestCase):
    ...
    def test_wildcard(self):
        cases = [
            ...
            [
                "foo/x%x/bar",
                ["foo", "bar", "fuz fuz fuz", "foo/*/bar",
                    "foo/xyz/bar", "foo/xx/baz"],
    ...
    def test_headerFormatter(self):
        ...
        for input, expected in cases:
            ...
            self.assertEqual(
                sorted(output.splitlines(True)), sorted(
    ...
    def test_fetchParserMacros(self):
        cases = [
            ...
            [
                b"FULL",
                (5, [b"flags", b"internaldate",
                 b"rfc822.size", b"envelope", b"body"]),
            ],
    ...
    def test_fetchParserBody(self):
        ...
        self.assertEqual(
            bytes(
                p.result[0]), b"BODY[HEADER.FIELDS.NOT (Subject Cc Message-Id)]"
        )
    ...
    def test_fetchParserMissingSectionClose(self):
        ...
        p = imap4._FetchParser()
        self.assertRaises(Exception, p.parseString,
    ...
    def test_queryBuilder(self):
        inputs = [
            ...
            imap4.Or(imap4.Query(deleted=1), imap4.Query(
                unseen=1), imap4.Query(new=1)),
            imap4.Or(
                imap4.Not(
                    imap4.Or(
                        imap4.Query(sorted=1, since="yesterday", smaller=1000),
                        imap4.Query(sorted=1, before="tuesday", larger=10000),
                        imap4.Query(sorted=1, unseen=1,
                                    deleted=1, before="today"),
    ...
    def test_parseIdList(self):
        ...
        for input, expected in zip(inputs, lengths):
            if expected is None:
                ...
                self.assertEqual(
                    L, expected, f"len({input!r}) = {L!r} != {expected!r}")
...
class IMAP4ServerTests(IMAP4HelperMixin, TestCase):
    ...
    def test_selectWithoutMailbox(self):
        ...
        self.connected.addErrback(
            self.assertClientFailureMessage, b"No such mailbox")
    ...
    def testCreate(self):
        ...
        self.result = []
        d1 = self.connected.addCallback(
            strip(login)).addCallback(strip(create))
        d2 = self.loopback()
        d = defer.gatherResults([d1, d2])
    ...
    def _cbTestCreate(self, ignored, succeed, fail):
        ...
        answers = sorted(
            ["inbox", "testbox", "test/box", "test", "test/box/box"])
    ...
    def testDelete(self):
        ...
        d.addCallback(
            lambda _: self.assertEqual(
                list(SimpleServer.theAccount.mailboxes), [])
        )
        return d
    ...
    def testDeleteWithInferiorHierarchicalNames(self):
        ...
        d.addCallback(
            lambda _: self.assertEqual(
                sorted(SimpleServer.theAccount.mailboxes), [
                    "DELETE", "DELETE/ME"]
            )
        )
        return d
    ...
    def testNonExistentDelete(self):
        ...
        d.addCallback(
            lambda _: self.assertEqual(
                str(self.failure.value), str(b"No such mailbox"))
        )
        return d
    ...
    def testIllegalDelete(self):
        ...
        d.addCallback(lambda _: self.assertEqual(
            str(self.failure.value), expected))
        return d
...
class IMAP4ServerSearchTests(IMAP4HelperMixin, TestCase):
    ...
    def test_searchSentBefore(self):
        ...
        """
        self.assertFalse(
            self.server.search_SENTBEFORE(
                self.earlierQuery, self.seq, self.msg)
        )
        self.assertTrue(
    ...
    def test_searchWildcard(self):
        ...
        self.assertTrue(self.server.search_UID(
            [b"2:*"], self.seq, self.msg, (1, 1234)))
        self.assertTrue(self.server.search_UID(
            [b"*"], self.seq, self.msg, (1, 1234)))
    ...
    def test_searchSentOn(self):
        ...
        self.assertFalse(self.server.search_SENTON(
            self.laterQuery, self.seq, self.msg))
    ...
    def test_searchNot(self):
        ...
        self.assertFalse(
            self.server.search_NOT(
                ["SENTSINCE"] +
                self.earlierQuery, self.seq, self.msg, (None, None)
            )
        )
        self.assertTrue(
            self.server.search_NOT(
    ...
    def test_searchBefore(self):
        ...
        self.assertTrue(self.server.search_BEFORE(
            self.laterQuery, self.seq, self.msg))
    ...
    def test_searchOn(self):
        ...
        """
        self.assertFalse(self.server.search_ON(
            self.earlierQuery, self.seq, self.msg))
        self.assertFalse(self.server.search_ON(
            self.sameDateQuery, self.seq, self.msg))
        self.assertFalse(self.server.search_ON(
            self.laterQuery, self.seq, self.msg))
    ...
    def test_searchSince(self):
        ...
        """
        self.assertTrue(self.server.search_SINCE(
            self.earlierQuery, self.seq, self.msg))
        self.assertTrue(
            self.server.search_SINCE(self.sameDateQuery, self.seq, self.msg)
        )
        self.assertFalse(self.server.search_SINCE(
            self.laterQuery, self.seq, self.msg))
...
class AuthenticatorTests(IMAP4HelperMixin, TestCase):
    ...
    def test_customChallengers(self):
        ...
        self.assertIn(base64.b64encode(
            special.getChallenge()), transport.value())
    ...
    def test_authNotBase64(self):
        ...
        self.assertIn(base64.b64encode(
            notBase64.getChallenge()), transport.value())
        ...
        self.assertEqual(
            transport.value(),
            b"".join([b"001 NO Authentication failed: ",
                     notBase64.message, b"\r\n"]),
        )
...
class UnsolicitedResponseTests(IMAP4HelperMixin, TestCase):
    ...
    def _cbTestNewMessagesAndRecent(self, ignored):
        E = self.client.events
        self.assertEqual(E, [["newMessages", 20, None],
                         ["newMessages", None, 10]])
...
class ClientCapabilityTests(TestCase):
    ...
    def test_categoryAtoms(self):
        ...
        self.protocol.dataReceived(
            b"* CAPABILITY IMAP4rev1 AUTH=LOGIN AUTH=PLAIN\r\n")
        self.protocol.dataReceived(b"0001 OK Capability completed.\r\n")

        def gotCapabilities(capabilities):
            self.assertEqual(
                capabilities, {b"IMAP4rev1": None,
                               b"AUTH": [b"LOGIN", b"PLAIN"]}
            )
    ...
    def test_mixedAtoms(self):
        ...
        def gotCapabilities(capabilities):
            self.assertEqual(
                capabilities,
                {b"IMAP4rev1": None, b"FOO": [
                    None, b"BAR"], b"BAR": [b"FOO", None]},
            )
...
class HandCraftedTests(IMAP4HelperMixin, TestCase):
    ...
    def test_fragmentedStringLiterals(self):
        ...
        self.assertEqual(transport.value(),
                         b"+ Ready for 8 octets of text\r\n")

        transport.clear()
        self.server.dataReceived(b"testuser {13}\r\n")
        self.assertEqual(transport.value(),
                         b"+ Ready for 13 octets of text\r\n")
    ...
    def test_literalWithoutPrecedingWhitespace(self):
        ...
        def test(result):
            ...
            self.assertEqual(
                result, {
                    1: [["BODY", ["HEADER.FIELDS", ["SUBJECT"]], "Hello"]]}
            )
    ...
    def test_flagsChangedInsideFetchSpecificResponse(self):
        ...
        def fetch():
            ...
            c.dataReceived(
                b'* 1 FETCH (BODY[HEADER.FIELDS ("SUBJECT")] {22}\r\n')
    ...
    def test_flagsChangedInsideFetchMessageResponse(self):
        ...
        def fetch():
            ...
            c.dataReceived(
                b"* 2 FETCH (FLAGS (\\Recent \\Seen) RFC822 {25}\r\n")
...
class SelectionTestsMixin(PreauthIMAP4ClientMixin):
    ...
    def test_exists(self):
        ...
        self.assertEqual(self.successResultOf(
            d), {"READ-WRITE": False, "EXISTS": 3})
    ...
    def test_uidvalidity(self):
        ...
        self.assertEqual(
            self.successResultOf(
                d), {"READ-WRITE": False, "UIDVALIDITY": 12345}
        )
...
class IMAP4ClientStoreTests(PreauthIMAP4ClientMixin, TestCase):
    ...
    def test_addFlagsSilentlyWithUnsolicitedData(self):
        ...
        """
        self._flagsSilentlyWithUnsolicitedDataTest(
            "addFlags", b"+FLAGS.SILENT")
...
class GetBodyStructureTests(TestCase):
    ...
    def test_singlePartWithMissing(self):
        ...
        structure = imap4.getBodyStructure(msg, extended=True)
        self.assertEqual(
            [major, minor, None, None, None, None,
                len(body), None, None, None, None],
            structure,
        )
...
class NewFetchTests(TestCase, IMAP4HelperMixin):
    ...
    def testFetchInternalDate(self, uid=0):
        ...
        self.messages = "13"
        self.msgObjs = [
            FakeyMessage({}, (), b"Fri, 02 Nov 2003 21:25:10 GMT",
                         b"", 23232, None),
            FakeyMessage({}, (), b"Thu, 29 Dec 2013 11:31:52 EST",
                         b"", 101, None),
            FakeyMessage({}, (), b"Mon, 10 Mar 1992 02:44:30 CST",
                         b"", 202, None),
            FakeyMessage({}, (), b"Sat, 11 Jan 2000 14:40:24 PST",
                         b"", 303, None),
        ]
        self.expected = {
    ...
    def testFetchSimplifiedBody(self, uid=0):
        ...
        self.expected = {
            0: {"BODY": [None, None, None, None, None, None, "12"]}}
    ...
    def testFetchBodyParts(self):
        ...
        self.msgObjs = [
            FakeyMessage(
                ...
                123,
                [
                    FakeyMessage(innerHeaders, (), None,
                                 innerBody1, None, None),
                    FakeyMessage(innerHeaders2, (), None,
                                 innerBody2, None, None),
                ],
            )
        ]
        self.expected = {
    ...
    def testFetchFast(self, uid=0):
        ...
        self.msgObjs = [
            FakeyMessage(
                {}, ("\\X",), b"19 Mar 2003 19:22:21 -0500", b"", 9, None),
        ]
        self.expected = {
            0: {
                "FLAGS": ["\\X"],
...
class CopyWorkerTests(TestCase):
    ...
    def testUnfeaturefulMessage(self):
        ...
        msgs = [
            FakeyMessage(
                {"Header-Counter": str(i)
                 }, (), b"Date", b"Body %d" % (i,), i + 10, None
            )
            for i in range(1, 11)
        ]
...
class TLSTests(IMAP4HelperMixin, TestCase):
    ...
    def testLoginLogin(self):
        self.server.checker.addUser(b"testuser", b"password-test")
        success = []
        self.client.registerAuthenticator(
            imap4.LOGINAuthenticator(b"testuser"))
        self.connected.addCallback(
    ...
    def startTLSAndAssertSession(self):
        ...
        def checkSecure(ignored):
            self.assertTrue(interfaces.ISSLTransport.providedBy(
...
class TimeoutTests(IMAP4HelperMixin, TestCase):
    ...
    def test_serverUnselectsMailbox(self):
        ...
        connection.
        """
        self.patch(SimpleServer.theAccount,
                   "mailboxFactory", UncloseableMailbox)
