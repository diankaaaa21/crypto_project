...
class WebSocketProtocol(WebSocketServerProtocol):
    ...
    def onConnect(self, request):
        ...
        try:
            ...
            if self.application_deferred is not None:
                self.application_deferred.addCallback(
                    self.applicationCreateWorked)
                self.application_deferred.addErrback(
                    self.applicationCreateFailed)
        except Exception:
    ...
    def applicationCreateWorked(self, application_queue):
        ...
        self.server.log_action(
            ...
            "connecting",
            {
                ...
                "client": (
                    "%s:%s" % tuple(
                        self.client_addr) if self.client_addr else None
                ),
            },
        )
    ...
    def serverSend(self, content, binary=False):
        ...
        if self.state == self.STATE_CONNECTING:
            self.serverAccept()
        logger.debug("Sent WebSocket packet to client for %s",
                     self.client_addr)
        if binary:
