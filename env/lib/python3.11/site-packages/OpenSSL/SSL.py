...
from OpenSSL.crypto import (FILETYPE_PEM, X509, PKey, X509Name, X509Store,
                            _EllipticCurve, _PassphraseHelper, _PrivateKey)
...
class _VerifyHelper(_CallbackExceptionHelper):
    ...
    def __init__(self, callback: _VerifyCallback) -> None:
        ...
        def wrapper(ok, store_ctx):  # type: ignore[no-untyped-def]
            ...
            try:
                result = callback(connection, cert,
                                  error_number, error_depth, ok)
...
class Context:
    ...
    def set_min_proto_version(self, version: int) -> None:
        ...
        """
        _openssl_assert(_lib.SSL_CTX_set_min_proto_version(
    ...
    def set_max_proto_version(self, version: int) -> None:
        ...
        version, this method will raise an exception.
        """
        _openssl_assert(_lib.SSL_CTX_set_max_proto_version(
            self._context, version) == 1)
    ...
    def set_default_verify_paths(self) -> None:
        ...
        *   manylinux cryptography wheels will work on most common Linux
        ...
        dir_env_var = _ffi.string(
            _lib.X509_get_default_cert_dir_env()).decode("ascii")
    ...
    def set_tlsext_use_srtp(self, profiles: bytes) -> None:
        ...
        _openssl_assert(_lib.SSL_CTX_set_tlsext_use_srtp(
            self._context, profiles) == 0)
    ...
    def set_alpn_protos(self, protos: list[bytes]) -> None:
        ...
        # with their lengths.
        protostr = b"".join(chain.from_iterable(
            (bytes((len(p),)), p) for p in protos))
        ...
        _openssl_assert(
            _lib.SSL_CTX_set_alpn_protos(
                self._context, input_str, len(protostr)) == 0
        )
...
class Connection:
    _reverse_mapping: typing.MutableMapping[Any,
                                            Connection] = WeakValueDictionary()
